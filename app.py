...
from models import (
    ...
    get_salespeople,
    insert_rfq_from_macro,
    insert_update,
    verify_rfq_and_lines,
)
...
from routes.emails import (
    emails_bp,
    get_company_name_by_email,
    sync_graph_mailbox_contacts,
)
...
from routes.rfqs import get_rfq_lines, rfqs_bp
from routes.sales_orders import sales_orders_bp
from routes.sales_suggestions import sales_suggestions_bp
from routes.salespeople import collect_customer_news, salespeople_bp
...
from routes.tickets import external_api_bp, tickets_bp
...
app.config["APOLLO_BASE_URL"] = (
    "https://api.apollo.io/v1"  # It's good to keep the base URL in config too
)
...
app.register_blueprint(
    excess_bp, url_prefix="/excess"
)  # Set up the route prefix for excess lists
...
def index():
    ...
    if current_user.is_authenticated:
        ...
        if salesperson_id:
            return redirect(
                url_for("salespeople.activity", salesperson_id=salesperson_id)
            )
...
def rfq_line_currency(line_id):
    current_app.logger.debug(
        f"Received request for RFQ line currency: line_id={line_id}"
    )
    try:
...
def add_rfq_from_outlook():
    ...
    if not customer_contact:
        return jsonify(
            {
                "status": "error",
                "message": "No matching contact or customer found for this email address",
            }
        ), 404
...
def scheduled_graph_email_cache_sync():
    ...
    with app.app_context():
        try:
            ...
            current_app.logger.exception(
                "Scheduled Graph Email Cache Sync failed: %s", exc
            )
