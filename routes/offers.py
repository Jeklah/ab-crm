...
from ai_helper import (
    extract_part_numbers_and_quantities,
    extract_quote_info,
    extract_quote_info_with_examples,
)
...
def create_offer():
    if request.method == "POST":
        ...
        with db_cursor(commit=True) as cursor:
            ...
            _execute_with_cursor(
                cursor, "SELECT currency FROM suppliers WHERE id = ?", (supplier_id,)
            )
...
@offers_bp.route(
    "/offers/<int:offer_id>/edit", methods=["GET", "POST"], endpoint="edit_offer"
)
def edit_offer(offer_id):
    ...
    manufacturers = db_execute(
        "SELECT id, name FROM manufacturers ORDER BY name", fetch="all"
    )
    ...
    for line in offer_lines_dicts:
        lookup_part_number = (
            line["requested_base_part_number"] or line["base_part_number"]
        )
    ...
    max_line_number = (
        max_line_row["max_line"] if max_line_row and max_line_row["max_line"] else 0
    )
...
@offers_bp.route(
    "/offers/<int:offer_id>/add_line", methods=["POST"], endpoint="add_offer_line"
)
    ...
    with db_cursor(commit=True) as cursor:
        for line in offer_lines:
            try:
                ...
                flash(
                    f"Error updating line {line['id']}: missing data for {e}.", "danger"
                )
                continue
...
@offers_bp.route(
    "/offers/<int:offer_id>/extract_offer_lines",
    methods=["POST"],
    endpoint="extract_offer_lines",
)
def extract_offer_lines(offer_id):
    offer_info = request.form.get("offer_info")
    print(
        f"Received offer_info for offer_id {offer_id}: {offer_info}"
    )  # Log the offer_info received
    ...
    try:
        with db_cursor(commit=True) as cursor:
            for line in extracted_lines:
                ...
                print(
                    f"Processing line: Part Number: {part_number}, Quantity: {quantity}, Price: {price}, Lead Time: {lead_time}, Manufacturer: {manufacturer}"
                )
                ...
                if not existing_part_number:
                    print(
                        f"Inserting new part number: {base_part_number}, {part_number}"
                    )
                    _execute_with_cursor(
                        cursor,
                ...
                print(
                    f"Inserted offer line {current_line_number} for offer_id {offer_id}"
                )
                current_line_number += 1

        print(f"Successfully inserted lines for offer {offer_id}")
        return jsonify(
            {"success": True, "message": "Offer lines extracted successfully"}
        ), 200
    except Exception as e:
...
def get_manufacturers_for_part():
    ...
    if manufacturers:
        return jsonify(
            manufacturers=[
                {"id": man["id"], "name": man["name"]} for man in manufacturers
            ]
        )
            ...
            with pdfplumber.open(filepath) as pdf:
                ...
                for page in pdf.pages:
                    ...
                    if page_text:
                        ...
                        logging.info(
                            f"No text found on page {pdf.pages.index(page) + 1}"
                        )
...
def manage_files(offer_id):
    ...
    return render_template(
        "offer_manage_files.html", files=files, offer=offer, offer_id=offer_id
    )
...
def view_pdf_text():
    ...
    if request.method == "POST":
        if "offer_info" in request.form:
            ...
            max_line_number = (
                max_line_row["max_line"]
                if max_line_row and max_line_row["max_line"]
                else 0
            )
            ...
            try:
                ...
                return redirect(
                    url_for("offers.view_pdf_text", offer_id=offer_id, file_id=file_id)
                )
    ...
    if file_id:
        ...
        file = db_execute(
            "SELECT * FROM files WHERE id = ?", (offer["file_id"],), fetch="one"
        )
    ...
    return render_template(
        "view_pdf_text.html", pdf_text=pdf_text, pdf_filename=pdf_filename, offer=offer
    )


@offers_bp.route(
    "/offers/<int:offer_id>/update_file", methods=["POST"], endpoint="update_offer_file"
)
...
def import_rfq_lines(offer_id):
    try:
        ...
        logging.debug(
            f"Importing RFQ lines for Offer ID: {offer_id} and Requisition ID: {req_id}"
        )
...
def upload_pdf_test(offer_id):
    ...
    return render_template(
        "pdf_column_mapping.html", extracted_table=None, offer_id=offer_id
    )
...
def map_columns(offer_id):
    ...
    extracted_table = session.get(
        "extracted_table"
    )  # Assuming the table is stored in session
...
@offers_bp.route(
    "/offers/<int:offer_id>/test_offer_lines",
    methods=["POST"],
    endpoint="test_offer_lines",
)
    ...
    try:
        ...
        with db_cursor(commit=True) as cursor:
            ...
            if not currency_id:
                ...
                supplier_currency = cursor.fetchone()
                currency_id = (
                    supplier_currency["currency"] if supplier_currency else None
                )
    ...
    for i in range(min(len(segments1), len(segments2))):
        ...
        if seg1 == seg2:
            ...
            if seg_similarity > 0.8:  # Very similar segments
                ...
                differences.append(
                    f"Segment {i + 1} differs significantly: {seg1} vs {seg2}"
                )
    ...
    try:
        ...
        for row in results:
            ...
            similarity, differences = calculate_part_similarity(
                normalized_input, row_dict["base_part_number"]
            )
            ...
            if similarity >= 0.6:  # Reduced threshold for more matches
                ...
                row_dict["normalized_match"] = normalize_part_number(
        ...
        return jsonify({"success": True, "rfq_lines": scored_results})
...
def get_customers_api():
    ...
    return jsonify({"customers": [dict(customer) for customer in customers]})
...
def process_offer_page():
    try:
        ...
        pdf_path = os.path.join(
            current_app.config["UPLOAD_FOLDER"], "NV-22413_2024.pdf"
        )
...
def process_offer_text():
    ...
    try:
        ...
        return jsonify({"success": True, "extracted_lines": extracted_lines})
    except Exception as e:
        ...
        with db_cursor(commit=True) as cursor:
            ...
            for attachment in msg.attachments:
                ...
                att_path = os.path.join(
                    current_app.config["UPLOAD_FOLDER"], att_filename
                )
...
def view_email_frame(offer_id):
    try:
        email_content = db_execute(
            "SELECT email_content FROM offers WHERE id = ?", (offer_id,), fetch="one"
        )
...
@offers_bp.route(
    "/rfq/<int:rfq_id>/compare", methods=["GET", "POST"], endpoint="compare_offers"
)
...
def show_offer_comparison_page(rfq_id):
    ...
    with db_cursor() as cursor:
        ...
        for line in rfq_lines:
            line["available_offers"] = get_available_offers(
                cursor, line, exact_match_only=True
            )
            for offer in line["available_offers"]:
                offer["extended_price"] = float(offer.get("price", 0)) * float(
                    line.get("quantity", 0)
                )
                ...
                if (
                    offer["min_line_value"] > 0
                    and offer["extended_price"] < offer["min_line_value"]
                ):
                    offer["below_min"] = True
                    if float(offer.get("price", 0)) > 0:
                        offer["min_quantity"] = math.ceil(
                            offer["min_line_value"] / float(offer.get("price", 0))
                        )
                    else:
...
def get_available_offers(cursor, line, exact_match_only=False):
        ...
        try:
            ...
            for offer in all_possible_offers:
                if offer.get("part_number"):
                    clean_offer_part = (
                        offer["part_number"].replace("-", "").replace(" ", "").upper()
                    )

                    # Check for similarity
                    if (
                        clean_part in clean_offer_part
                        or clean_offer_part in clean_part
                        or (
                            len(clean_part) > 3
                            and clean_part[:4] == clean_offer_part[:4]
                        )
                    ):
    ...
    if not all_offers:
        try:
            ...
            if last_resort:
                print(
                    f"Using last resort: showing {len(last_resort)} offers for {base_part_number}"
                )
    ...
    print(
        f"Final result: {len(unique_offers)} unique valid offers for {base_part_number}"
    )
...
def search_for_similar_parts(cursor, part_number):
    ...
    for offer in all_offers:
        if offer.get("part_number"):
            ...
            if (
                part_pattern in offer_pattern
                or offer_pattern in part_pattern
                or (len(part_pattern) > 3 and part_pattern[:4] == offer_pattern[:4])
            ):
    ...
    for offer in line["available_offers"]:
        ...
        if (
            ...
            if float(offer["price"]) > 0:
                offer["min_quantity"] = math.ceil(
                    offer["min_line_value"] / float(offer["price"])
                )
            else:
...
def handle_submit_selected_offers(rfq_id):
    ...
    try:
        ...
        flash(
            "Offers successfully applied to RFQ lines with delivery costs amortized",
            "success",
        )
...
def extract_supplier_settings(form_data):
    ...
    for key, value in form_data.items():
        if key.startswith("delivery_cost_"):
            ...
            supplier_settings[supplier_id]["delivery_cost"] = (
            ...
            if supplier_id not in supplier_settings:
                supplier_settings[supplier_id] = {}
            supplier_settings[supplier_id]["min_line_value"] = (
                float(value) if value else 0
            )
...
def update_supplier_settings(cursor, supplier_settings):
    ...
    if "delivery_cost" not in columns:
        cursor.execute(
            "ALTER TABLE suppliers ADD COLUMN delivery_cost NUMERIC(10, 2) DEFAULT 0"
        )
        columns.add("delivery_cost")
    if "minimum_line_value" not in columns:
        cursor.execute(
            "ALTER TABLE suppliers ADD COLUMN minimum_line_value NUMERIC(10, 2) DEFAULT 0"
        )
        ...
        if key.startswith("line_"):
            ...
            if len(offer_data) >= 2:
                ...
                supplier_lines[supplier_id].append(
                    {"line_id": line_id, "offer_id": offer_id}
                )
    ...
    for supplier_id, lines in supplier_lines.items():
        ...
        for line_item in lines:
            update_single_rfq_line(
                cursor, line_item, supplier_id, per_line_delivery, min_line_value
            )


def update_single_rfq_line(
    cursor, line_item, supplier_id, per_line_delivery, min_line_value
):
    ...
    # Get RFQ line data
    rfq_line = cursor.execute(
        "SELECT quantity FROM rfq_lines WHERE id = ?", (line_id,)
    ).fetchone()
    ...
    if min_line_value > 0 and extended_price < min_line_value:
        ...
        print(
            f"Line {line_id} below minimum value ({extended_price} < {min_line_value})"
        )

    # Get currency and buffer data from supplier
    cursor.execute(
        "SELECT currency, buffer FROM suppliers WHERE id = ?", (supplier_id,)
    )
            ...
            for line in rfq_lines:
                ...
                if include_key in request.form:
                    ...
                    if price and float(price) > 0:
                        ...
                        max_line = (
                            max_row["max_line"]
                            if max_row and max_row["max_line"]
                            else 0
                        )
...
def add_multiple_offer_lines(offer_id):
    ...
    try:
        ...
        with db_cursor(commit=True) as cursor:
            _execute_with_cursor(
                cursor, "SELECT id FROM offers WHERE id = ?", (offer_id,)
            )
