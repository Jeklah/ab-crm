...
from ai_helper import (
    apply_perplexity_enrichment,
    enrich_customer_data,
    enrich_customer_with_perplexity,
    generate_industry_insights,
    generate_industry_insights_with_custom_prompt,
    generate_preview_prompt,
    start_bulk_enrichment,
    start_perplexity_enrichment,
...
from models import (
    ...
    delete_customer_industries,
    delete_customer_tags,
    get_addresses_by_customer,
    get_all_company_types,
    get_all_contact_lists,
    get_all_contact_status_counts,
    get_all_contact_statuses,
    get_all_contacts_by_status,
    get_all_contacts_filtered,
    get_all_customers,
    get_all_tags,
    get_available_company_types,
    ...
    update_contact,
    update_contact_list_name,
    update_contact_status,
    update_customer,
    update_customer_apollo_id,
    update_customer_development_answer,
    update_customer_enrichment,
    update_customer_industry,
)
...
def _using_postgres():
    return bool(
        _DATABASE_URL and _DATABASE_URL.startswith(("postgres://", "postgresql://"))
    )
...
def _get_customer_permission_flags(customer_data):
    ...
    can_edit = (
        ...
        or (user_salesperson_id and user_salesperson_id == customer_salesperson_id)
    )
    can_view = (
        can_edit
...
def _get_customer_supplier_ids(customer_id):
    ...
    return [
        int(row["supplier_id"]) for row in rows if row.get("supplier_id") is not None
    ]
...
def customers():
    if current_user.is_administrator() or current_user.can(Permission.VIEW_CUSTOMERS):
        ...
        salespeople = (
            [get_salesperson_by_id(current_user.get_salesperson_id())]
            if current_user.get_salesperson_id()
            else []
        )
    ...
    breadcrumbs = generate_breadcrumbs(
        ("Home", url_for("index")), ("Customers", url_for("customers.customers"))
    )
...
def edit_customer(customer_id):
    ...
    if "salesperson_id" in customer_dict and customer_dict["salesperson_id"]:
        ...
        if salesperson:
            ...
            customer_dict["salesperson_name"] = salesperson_dict.get(
                "name", "Unassigned"
            )
        else:
    ...
    if request.method == "POST":
        ...
        try:
            ...
            print(
                f"Primary Contact ID: {request.form.get('primary_contact_id')}",
                flush=True,
            )
            ...
            country = request.form.get(
                "country", ""
            ).upper()  # Store as uppercase ISO code
            system_code = request.form.get(
                "system_code", ""
            ).strip()  # Added system_code
            status_id_raw = request.form.get("status_id")
            try:
                status_id = (
                    int(status_id_raw)
                    if status_id_raw and status_id_raw.strip()
                    else None
                )
            ...
            try:
                currency_id = (
                    int(currency_raw) if currency_raw and currency_raw.strip() else None
                )
            except (ValueError, TypeError):
                currency_id = None
...
def get_customer_notes(customer_id):
    result = db_execute(
        "SELECT notes FROM customers WHERE id = ?", (customer_id,), fetch="one"
    )
    if not result:
        return ""
...
def customer_development(customer_id):
    ...
    breadcrumbs = generate_breadcrumbs(
        ...
        (
            "Development Plan",
            url_for("customers.customer_development", customer_id=customer_id),
        ),
    )

    return render_template(
        "customer_development.html",
        customer=customer_dict,
        development_plan=development_plan,
        breadcrumbs=breadcrumbs,
        can_edit=can_edit,
...
def update_development_answer(customer_id):
    ...
    try:
        ...
        if not development_point_id:
            return jsonify(
                {"success": False, "error": "Missing development point ID"}
            ), 400
        ...
        if answer:
            ...
            success = delete_customer_development_answer(
...
def add_contact():
    ...
    try:
        ...
        if not data.get("name") or not data.get("email") or not data.get("customer_id"):
            return jsonify(
                {"success": False, "error": "Name, email, and customer_id are required"}
            )
...
def update_contact_notes(contact_id):
    try:
        ...
        contact = db_execute(
            "SELECT id FROM contacts WHERE id = ?", (contact_id,), fetch="one"
        )
        if contact is None:
...
def update_contact_customer_id(contact_id, customer_id):
    db_execute(
        "UPDATE contacts SET customer_id = ? WHERE id = ?",
...
def add_update(customer_id):
    ...
    print(
        f"DEBUG: X-Requested-With header: '{request.headers.get('X-Requested-With')}'"
    )
    ...
    try:
        ...
        if update_type in ["email", "phone"] and not update_text:
            ...
            if contact_id:
                ...
                update_text = (
                    "Emailed customer" if update_type == "email" else "Called customer"
                )
        ...
        print(
            f"  - communication_type: {update_type if update_type != 'generic' else None}"
        )
        print(
            f"  - update_date: {parsed_datetime}"
        )  # CHANGED: Now using parsed_datetime
...
def edit_update(update_id):
    ...
    try:
        with db_cursor(commit=True) as cursor:
            ...
            if (
                not user_salesperson
                or user_salesperson["legacy_salesperson_id"]
                != current_update["salesperson_id"]
            ):
                ...
                return redirect(
                    url_for(
                        ...
                        customer_id=current_update["customer_id"],
                    )
                )
            ...
            if contact_id:
                cursor.execute(
                    "SELECT customer_id, name FROM contacts WHERE id = ?", (contact_id,)
                )
                ...
                print(
                    f"DEBUG: Contact {contact_info['name']} belongs to customer {new_customer_id}"
                )

                if communication_type in ["email", "phone"] and (
                    not update_text
                    or update_text in ["Emailed customer", "Called customer"]
                ):
            ...
            if current_update["communication_id"]:
                if contact_id and communication_type:
                    ...
                    cursor.execute(
                        "DELETE FROM contact_communications WHERE id = ?",
                        (current_update["communication_id"],),
                    )
...
def delete_update(update_id):
    ...
    try:
        with db_cursor(commit=True) as cursor:
            ...
            if (
                ...
                return redirect(
                    url_for(
                        "customers.edit_customer",
                        customer_id=update_info["customer_id"],
                    )
                )
        ...
        return redirect(
            url_for("customers.edit_customer", customer_id=update_info["customer_id"])
        )
    except Exception as e:
...
def prospecting():
    ...
    if watched_tags_response.status_code == 200:
        watched_tags_data = (
            watched_tags_response.json()
        )  # Convert API JSON response to Python list
    ...
    if selected_continent:
        available_countries = get_available_countries(
            selected_continent, selected_tag_id
        )
    ...
    if is_ajax:
        ...
        if request.method == "GET":
            return jsonify({"available_countries": available_countries})
        ...
        if request.method == "POST" and "generate_insights" in request.form:
            try:
                ...
                return jsonify({"success": True, "html": insights_html})
    ...
    breadcrumbs = generate_breadcrumbs(
        ("Home", url_for("index")), ("Prospecting", url_for("customers.prospecting"))
    )
    ...
    try:
        ...
        for customer in customers:
            ...
            customer["assigned_salesperson_name"] = data.get(
...
def enrich_customer():
    ...
    try:
        ...
        customer, current_tags, current_company_types = get_customer_data(
            data["customer_id"]
        )
        ...
        customer_data = {
            ...
            "current_company_types": [
                ct["name"] for ct in (current_company_types or [])
            ],
        }
...
def match_apollo_organization(customer_id):
    ...
    try:
        response = requests.get(
            ...
            params={"api_key": current_app.config["APOLLO_API_KEY"]},
        )
        ...
        if response.status_code == 200:
            ...
            try:
                ...
                return jsonify(
                    {"success": True, "message": "Successfully matched with Apollo"}
                )
            except Exception as e:
...
def get_customer_leads(customer_id):
    ...
    try:
        current_app.logger.info(
            f"Searching Apollo for leads at company ID: {customer['apollo_id']}"
        )
...
def enrich_person():
    ...
    try:
        ...
        if not phone:
            ...
            if phone_numbers:
                phone = phone_numbers[0].get("sanitized_number") or phone_numbers[
                    0
                ].get("raw_number")
        if not phone:
            ...
            if not phone:
                ...
                if contact_numbers:
                    phone = contact_numbers[0].get(
                        "sanitized_number"
                    ) or contact_numbers[0].get("raw_number")
...
def list_customers():
    try:
        ...
        for customer in customers:
            ...
            last_email = (
                result["last_email"] if result and result["last_email"] else None
            )
        ...
        return jsonify({"success": True, "customers": customer_data})
...
def get_matching_templates(customer_id):
    ...
    return jsonify({"success": True, "templates": matching_templates})
...
def bump_status(customer_id):
    ...
    try:
        ...
        available_statuses = db_execute(
            "SELECT id, status FROM customer_status ORDER BY id", fetch="all"
        )
        ...
        customer = db_execute(
            "SELECT status_id FROM customers WHERE id = ?", (customer_id,), fetch="one"
        )
        ...
        new_status = next(
            (s for s in available_statuses if s["id"] == next_status_id), None
        )
...
def bump_priority(customer_id):
    ...
    try:
        ...
        available_priorities = db_execute(
            "SELECT id, name, color FROM priorities ORDER BY id", fetch="all"
        )
        ...
        customer = db_execute(
            "SELECT priority FROM customers WHERE id = ?", (customer_id,), fetch="one"
        )
        ...
        new_priority = next(
            (p for p in available_priorities if p["id"] == next_priority_id), None
        )
        ...
        return jsonify(
            {
                ...
                "new_priority": {
                    ...
                    if new_priority
                    else f"Priority {next_priority_id}",
...
def customer_search():
    ...
    filter_to_parts_lists = str(request.args.get("has_parts_list", "")).lower() not in (
        "",
        "0",
        "false",
        "none",
    ...
    if filter_to_parts_lists:
        additional_where = (
            " AND EXISTS (SELECT 1 FROM parts_lists pl WHERE pl.customer_id = c.id"
        )
...
def get_timeline(customer_id):
    ...
    try:
        with db_cursor() as cursor:
            ...
            current_app.logger.info(
                f"Fetching contact emails for customer {customer_id}"
            )
            cursor.execute(
                """
            ...
            current_app.logger.info(
                f"Found {len(contact_emails)} contact emails in {time.time() - contact_start:.2f}s"
            )
            ...
            current_app.logger.info(
                f"Timeline query returned {len(rows)} rows in {time.time() - timeline_start:.2f}s"
            )
            ...
            current_app.logger.info(
                f"Processed results in {time.time() - processing_start:.2f}s"
            )
            ...
            # Count emails
            cursor.execute(
                f"SELECT COUNT(*) FROM emails WHERE {email_conditions_sql}", params
            )
            email_count = cursor.fetchone()[0]

            # Count RFQs
            cursor.execute(
                "SELECT COUNT(*) FROM rfqs WHERE customer_id = ?", (customer_id,)
            )
            ...
            current_app.logger.info(
                f"Count query completed in {time.time() - count_start:.2f}s"
            )
            current_app.logger.info(
            ...
            current_app.logger.info(
                f"Total timeline request completed in {total_time:.2f}s"
            )
...
def get_emails(customer_id):
    ...
    try:
        with db_cursor() as cursor:
            ...
            contact_emails = [row[0] for row in cursor.fetchall()]
            current_app.logger.info(
                f"Found {len(contact_emails)} contact emails in {time.time() - start_time:.2f}s"
            )
            current_app.logger.debug(f"Contact emails: {contact_emails}")
        ...
        rows = cursor.fetchall()
        current_app.logger.info(
            f"Timeline query returned {len(rows)} rows in {time.time() - query_start:.2f}s"
        )
        ...
        current_app.logger.info(
            f"Processed results in {time.time() - process_start:.2f}s"
        )
        ...
        current_app.logger.info(
            f"Total email request completed in {time.time() - start_time:.2f}s"
        )
...
def get_rfqs(customer_id):
    ...
    try:
        with db_cursor() as cursor:
            ...
            rfqs = [
                {"id": row[0], "date": row[1], "reference": row[2], "status": row[3]}
                for row in rows
            ]
...
def get_customers_data():
    try:
        ...
        rows = db_execute(
            query,
            [search_lower, like_param, like_param, per_page, offset],
            fetch="all",
        )
        ...
        for row in rows:
            ...
            if row["primary_contact_name"] and row["primary_contact_email"]:
                customer_dict["primary_contact"] = (
                    f"{row['primary_contact_name']} ({row['primary_contact_email']})"
                )
            else:
...
def get_customer_insights(customer_id):
    try:
        ...
        month_labels = [
            _add_months(start_month, idx).strftime("%Y-%m") for idx in range(12)
        ]
        ...
        monthly_sales = [
            {"month": label, "total_value": monthly_totals[label]}
            for label in month_labels
        ]
        yearly_sales = [
            {"year": year, "total_value": yearly_totals[year]} for year in year_labels
        ]
                        ...
                        "datasets": [
                            {
                                "label": "Sales Value",
                                "data": [
                                    entry["total_value"] for entry in monthly_sales
                                ],
                            }
                        ],
                    },
                        ...
                        "datasets": [
                            {
                                "label": "Annual Sales",
                                "data": [
                                    entry["total_value"] for entry in yearly_sales
                                ],
                            }
                        ],
                    },
...
def filter_customers():
    try:
        ...
        countries = [
            country for country in request.args.getlist("countries") if country
        ]
        ...
        if filter_salesperson:
            ...
            if salesperson_ids:
                placeholders = ",".join(
                    f":sp_id_{i}" for i in range(len(salesperson_ids))
                )
        ...
        tags_agg_fn = (
            "STRING_AGG(it.tag, ', ')" if _using_postgres() else "GROUP_CONCAT(it.tag)"
        )
        ...
        ytd_sales = {}
        start_of_year = datetime.utcnow().replace(
        ...
        with db_cursor() as cur:
            ...
            if customer_ids:
                ...
                ytd_sales = {
                    row["customer_id"]: row["ytd_sales"] for row in ytd_results
                }
        ...
        for row in rows:
            ...
            if row["budget"] is not None:
                customer_dict["budget"] = {"amount": row["budget"], "currency": "EUR"}
            else:
            ...
            if row["update_date"]:
                customer_dict["latest_update"] = {
                    "date": row["update_date"].isoformat()
                    if isinstance(row["update_date"], datetime)
                    else row["update_date"],
    ...
    try:
        with db_cursor() as cur:
            ...
            if activity_type == "email":
                ...
                if not message_id:
                    return jsonify(
                        {"success": False, "error": "Email message_id not found"}
                    ), 404
                ...
                result, message_numbers = mail.search(
                    None, f'HEADER Message-ID "{search_message_id}"'
                )
                full_body = None
                if result == "OK" and message_numbers and message_numbers[0]:
                    ...
                    if result == "OK" and msg_data and len(msg_data) > 0:
                        ...
                        if msg_obj.is_multipart():
                            ...
                            full_body = msg_obj.get_payload(decode=True).decode(
                                errors="replace"
                            )
                ...
                lines_query = """
                    ...
                    WHERE rfq_id = ?
                """
                line_rows = _execute_with_cursor(
                    cur, lines_query, (activity_id,)
                ).fetchall()
                ...
                return jsonify(
                    {"success": False, "error": "Unsupported activity type"}
                ), 400
...
def enrich_dashboard():
    ...
    return render_template(
        "customers/enrich/dashboard.html", stats=stats, recent=recent
    )
...
def get_enrichment_status():
    ...
    with db_cursor() as cur:
        ...
        industry_tags_agg = (
            "STRING_AGG(DISTINCT it.tag, ', ')"
            if _using_postgres()
            else "GROUP_CONCAT(DISTINCT it.tag)"
        )
        company_types_agg = (
            "STRING_AGG(DISTINCT ct.type, ', ')"
            if _using_postgres()
            else "GROUP_CONCAT(DISTINCT ct.type)"
        )
...
def view_suggestions():
    ...
    with db_cursor() as cur:
        ...
        suggestions = _execute_with_cursor(
            cur, suggestions_query, (reviewed_false,)
        ).fetchall()
...
def stop_enrichment():
    ...
    return jsonify({"status": "success", "message": "Enrichment process stopped"})
...
def approve_tag_suggestion():
    ...
    try:
        ...
        if not data or "tag" not in data:
            return jsonify({"status": "error", "message": "Missing tag data"}), 400
        ...
        with db_cursor(commit=True) as cur:
            ...
            tag_row = _execute_with_cursor(
                cur, "SELECT id FROM industry_tags WHERE tag = ?", (data["tag"],)
            ).fetchone()
...
def reject_tag_suggestion():
    ...
    try:
        ...
        return jsonify(
            {"status": "success", "message": f'Tag "{data["tag"]}" rejected'}
        )
            ...
            customer_data.append(
                {
                    ...
                    "assigned_salesperson_name": row_dict.get(
                        "assigned_salesperson_name"
                    ),
...
def market_coverage():
    countries_agg = (
        "STRING_AGG(DISTINCT c.country, ', ')"
        if _using_postgres()
        else "GROUP_CONCAT(DISTINCT c.country)"
    )
    sales_cutoff = (
...
def get_tag_analysis(tag_id):
    ...
    try:
        ...
        analysis_text = re.sub(
            r"<think>.*?</think>", "", analysis_text, flags=re.DOTALL
        )
...
def get_tag_customers(tag_id):
    try:
        ...
        with db_cursor() as cur:
            ...
            print(
                "First customer data from DB:",
                dict(customers[0]) if customers else None,
            )
        ...
        return jsonify(
            {"customers": [dict(row) for row in customers], "total_pages": total_pages}
        )
    ...
    try:
        ...
        for customer in existing_customers:
            ...
            if country not in country_breakdown:
                country_breakdown[country] = {"customer_count": 0, "customers": []}
        ...
        if os.environ.get("OPENAI_API_KEY"):
            ...
            try:
                ...
                formatted_analysis = format_with_openai(
                    raw_analysis, tag_info, country_breakdown
                )
            except Exception as e:
                import logging
...
def lookup_country_code():
    ...
    try:
        ...
        if not country:
            for c in countries:
                if (
                    hasattr(c, "official_name")
                    and c.official_name.lower() == country_name.lower()
                ):
                    country = c
                    break
        ...
        if country:
            return jsonify({"code": country.alpha_2, "name": country.name})
        else:
    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are a data parsing assistant. Return only valid JSON without any explanation or markdown formatting.",
                },
...
def natural_language_search():
    ...
    try:
        ...
        with db_cursor() as cur:
            existing_customers = [
                row["name"]
                for row in _execute_with_cursor(
                    cur, "SELECT name FROM customers"
                ).fetchall()
            ]
        ...
        flash(
            "Sorry, I couldn't process that search. Please try rephrasing it.",
            "warning",
        )
        return redirect(url_for("customers.prospecting"))
...
def suggest_contacts(customer_id):
    ...
    try:
        ...
        with db_cursor() as cur:
            ...
            current_app.logger.info(
                f"Customer has {len(existing_emails)} existing contacts"
            )
            ...
            for domain in domains:
                ...
                sender_rows = _execute_with_cursor(
                    cur, sender_query, (f"%@{domain_lower}",)
                ).fetchall()
                ...
                recipient_rows = _execute_with_cursor(
                    cur, recipient_query, (f"%@{domain_lower}",)
                ).fetchall()
                ...
                for row in list(sender_rows) + list(recipient_rows):
                    ...
                    if email in email_data:
                        ...
                        email_data[email]["first_seen"] = min(
                            email_data[email]["first_seen"], row["first_seen"]
                        )
                        email_data[email]["last_seen"] = max(
                            email_data[email]["last_seen"], row["last_seen"]
                        )
                    else:
                        email_data[email] = {
                            "email": email,
                ...
                for email, data in email_data.items():
                    ...
                    data["recent_subjects"] = [
                        subj["subject"] for subj in recent_subjects
                    ]
                    suggestions.append(data)

            suggestions.sort(
                key=lambda x: (x["email_count"], x["last_seen"]), reverse=True
            )

        return jsonify({"success": True, "suggested_contacts": suggestions})
...
@customers_bp.route(
    "/customers/activity/<activity_type>/<int:activity_id>", methods=["GET"]
)
...
def add_contact_communication(customer_id):
    ...
    with db_cursor() as cur:
        ...
        contact_id = request.form.get("contact_id")
        communication_type = request.form.get(
            "communication_type"
        )  # 'email', 'phone', etc.
        notes = request.form.get("notes", "")
        ...
        contact = _execute_with_cursor(
            cur, "SELECT name, email FROM contacts WHERE id = ?", (contact_id,)
        ).fetchone()
    ...
    insert_update(
        customer_id, salesperson_id, update_text, contact_id, communication_type
    )
...
def get_customer_contacts_api(customer_id):
    ...
    try:
        ...
        if not current_user.is_administrator() and not current_user.can(
            ...
            if (
                not customer
                or customer["salesperson_id"] != current_user.get_salesperson_id()
            ):
        ...
        return jsonify({"success": True, "contacts": serializable_contacts})
    except Exception as e:
...
def contacts_page(salesperson_id):
    with db_cursor() as cur:
        ...
        customers = _execute_with_cursor(
            cur, "SELECT id, name FROM customers ORDER BY name"
        ).fetchall()
    ...
    return render_template(
        "contacts.html", contacts=contacts, customers=customers, breadcrumbs=breadcrumbs
    )
...
def delete_contact(contact_id):
    try:
        with db_cursor(commit=True) as cur:
            _execute_with_cursor(
                cur, "DELETE FROM contacts WHERE id = ?", (contact_id,)
            )
...
@customers_bp.route(
    "/api/contact_lists/<int:list_id>", methods=["GET", "PUT", "DELETE"]
)
def handle_contact_list(list_id):
    ...
    try:
        if request.method == "GET":
            ...
            return jsonify(
                {"success": True, "contacts_added": added, "contacts_removed": removed}
            )
...
@customers_bp.route(
    "/api/contact_lists/<int:list_id>/contacts", methods=["POST", "DELETE"]
)
...
def contact_status_summary():
    ...
    try:
        ...
        return render_template(
            "customers/partials/status_summary.html", status_counts=status_counts
        )
...
def all_contacts():
    ...
    try:
        ...
        contacts = get_all_contacts_filtered(
            search_term, customer_filter, status_filter
        )
    ...
    try:
        ...
        with db_cursor() as cur:
            associated_results = _execute_with_cursor(
                cur, associated_query, (customer_id,)
            ).fetchall()
        ...
        for contact in contacts:
            ...
            if (
                contact_customer_id != customer_id
                and contact_customer_id in associated_companies
            ):
    ...
    try:
        with db_cursor(commit=True) as cur:
            current_contact = _execute_with_cursor(
                cur, "SELECT status_id FROM contacts WHERE id = ?", (contact_id,)
            ).fetchone()
            ...
            statuses = _execute_with_cursor(
                cur, "SELECT id, name, color FROM contact_statuses ORDER BY id"
            ).fetchall()

            if not statuses:
                return jsonify(
                    {"success": False, "error": "No contact statuses available"}
                )
...
def get_contact_details(contact_id):
    ...
    is_ajax_request = (
        ...
        or is_from_customer_edit  # If called from customer edit page, treat as AJAX
    )
    ...
    try:
        ...
        if not contact:
            ...
            if is_ajax_request:
                ...
                response_data = {"success": False, "error": "Contact not found"}
        ...
        if is_ajax_request:
            response_data = {"success": True, "contact": contact}
        ...
        flash(
            f"Viewing contact: {contact_dict.get('name', 'Unknown')} ({contact_dict.get('email', '')})",
            "info",
        )
        return redirect(
            url_for("customers.edit_customer", customer_id=contact_dict["customer_id"])
        )
        ...
        if is_ajax_request:
            ...
            error_response = {"success": False, "error": "Failed to get contact"}
...
def get_contact_statuses_route():
    ...
    try:
        ...
        return jsonify(
            {"success": False, "error": "Failed to get contact statuses"}
        ), 500
...
def contact_search():
    ...
    full_name_expr = (
        "concat_ws(' ', c.name, c.second_name)"
        if _using_postgres()
        else "(c.name || ' ' || COALESCE(c.second_name, ''))"
    )

    salesperson_id = (
        request.args.get("salesperson_id", type=int)
        or current_user.get_salesperson_id()
    )
...
def customer_development_api(customer_id):
    ...
    try:
        ...
        completed_points = sum(
            1 for point in plan_data if point["answer"] and point["answer"].strip()
        )
        ...
        return jsonify(
            {
                ...
                "progress": {"total": total_points, "completed": completed_points},
                "customer": {
                    "id": customer_id,
        ...
        return jsonify(
            {"success": False, "error": "Failed to load development plan"}
        ), 500
...
def get_contact_communications_api(contact_id):
    ...
    try:
        ...
        with db_cursor() as cur:
            contact = _execute_with_cursor(
                cur, "SELECT customer_id FROM contacts WHERE id = ?", (contact_id,)
            ).fetchone()
        ...
        if customer:
            ...
            can_view = (
                ...
                or (
                    user_salesperson_id
                    and user_salesperson_id == customer_salesperson_id
                )
            )
        ...
        for comm in communications:
            result.append(
                {
                    "id": comm.get("id"),
                    "communication_type": comm.get(
                        "type", comm.get("communication_type", "Other")
                    ),
                    "communication_date": comm["date"].isoformat()
                    if isinstance(comm.get("date"), datetime)
                    else comm.get("date", ""),
...
def add_customer_association_api(customer_id):
    ...
    try:
        ...
        if not associated_customer_id:
            return jsonify(
                {"success": False, "error": "Associated customer ID is required"}
            ), 400
        ...
        if existing:
            return jsonify(
                {"success": False, "error": "This association already exists"}
            ), 400

        # Prevent self-association
        if customer_id == associated_customer_id:
            return jsonify(
                {"success": False, "error": "Cannot associate a customer with itself"}
            ), 400
        ...
        return jsonify(
            {
                ...
                "association": {
                    ...
                    "associated_customer_id": created_association[
                        "associated_customer_id"
                    ],
                    "associated_customer_name": created_association[
...
@customers_bp.route(
    "/<int:customer_id>/associations/<int:association_id>/api", methods=["DELETE"]
)
@login_required
def delete_customer_association_api(customer_id, association_id):
    ...
    try:
        ...
        db_execute(
            "DELETE FROM customer_associations WHERE id = ?",
            (association_id,),
            commit=True,
        )

        return jsonify({"success": True, "message": "Association deleted successfully"})
...
@customers_bp.route(
    "/<int:customer_id>/associations/<int:association_id>/api", methods=["PUT"]
)
@login_required
...
def get_customer_associations_api(customer_id):
    ...
    try:
        ...
        """
        print(
            "CUSTOMER ASSOCIATIONS (subsidiaries):", query, (customer_id,), flush=True
        )
        ...
        """
        print(
            "CUSTOMER ASSOCIATIONS (parents):", parent_query, (customer_id,), flush=True
        )
...
def customer_overview(customer_id):
    ...
    try:
        ...
        yearly_spend = sorted(
            yearly_spend_dict.values(), key=lambda item: int(item["year"])
        )

        for update in recent_updates:
            activity_type = (
                update["communication_type"]
                if update["communication_type"]
                else "update"
            )
        ...
        overview_data = {
            ...
            "last_purchase": {
                ...
                "value": float(last_purchase["value"])
                if last_purchase and last_purchase["value"]
                else 0,
            }
            if last_purchase
            else None,
            "lifetime_value": float(lifetime_stats["lifetime_value"])
            if lifetime_stats
            else 0,
            "total_orders": int(lifetime_stats["total_orders"])
            if lifetime_stats
            else 0,
            "avg_order_value": float(lifetime_stats["avg_order_value"])
            if lifetime_stats
            else 0,
            "last_contact": {
...
def enrich_single_customer(customer_id):
    ...
    try:
        ...
        tags_rows = (
            db_execute(
                "SELECT id, tag as name, description FROM industry_tags", fetch="all"
            )
            or []
        )
        ...
        types_rows = (
            db_execute("SELECT id, type as name FROM company_types", fetch="all") or []
        )
