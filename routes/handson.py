...
from models import (
    ...
    get_excess_stock_list_id_by_file,
    get_file_by_id,
    get_part_numbers,
    insert_excess_stock_line,
    insert_part_number,
)  # Import your function to get data
...
def _using_postgres():
    return bool(
        os.getenv("DATABASE_URL", "").startswith(("postgres://", "postgresql://"))
    )
...
def _is_unique_violation(exc):
    ...
    if getattr(exc, "pgcode", None) == "23505":
        return True
    # Fallback: check the error message for common unique constraint wording
    msg = str(exc).lower()
    return "unique" in msg and (
        "constraint" in msg or "violation" in msg or "duplicate" in msg
    )
...
def process_excess_list():
    ...
    if "excess_stock_list_id" not in content:
        return jsonify(
            success=False, message="'excess_stock_list_id' is missing from the request"
        ), 400
    ...
    if quantity_index is None or (
        base_part_index is None and part_number_index is None
    ):
        return jsonify(
            success=False,
            message="Mapping error: quantity and part_number or base_part_number are required",
        ), 400
    ...
    for row_index, row in enumerate(data):
        try:
            ...
            if (
                unit_price_currency_index is not None
                and unit_price_currency_index >= len(row)
            ):
            ...
            raw_part_number = (
                ...
                if part_number_index is not None
                else row[base_part_index]
            )
            ...
            if quantity is None:
                continue
            manufacturer = (
                row[manufacturer_index] if manufacturer_index is not None else None
            )
            date_code = row[date_code_index] if date_code_index is not None else None
            unit_price = (
                _parse_unit_price(row[unit_price_index])
            ...
            if base_part_number not in existing_part_numbers:
                ...
                insert_part_number(
                    part_number=raw_part_number, base_part_number=base_part_number
                )
                existing_part_numbers.add(
                    base_part_number
                )  # Update the set with the new base part number

            # Insert into the excess_stock_lines table
            insert_excess_stock_line(
                excess_stock_list_id=excess_stock_list_id,  # Ensure this ID is passed correctly
            ...
            return jsonify(
                success=False, message=f"Error processing row: {str(e)}"
            ), 500
    ...
    return jsonify(
        success=True, redirect_url=f"/excess/excess_lists/{excess_stock_list_id}/edit"
    )
...
def excess_list_mapping(file_id):
    ...
    show_all = str(request.args.get("show_all", "")).lower() in (
        "1",
        "true",
        "on",
        "yes",
    ...
    if file_path.endswith(".xls") or file_path.endswith(".xlsx"):
        ...
        excess_stock_list_id = get_excess_stock_list_id_by_file(
            file_id
        )  # Define your logic for retrieving this
        excess_list = (
            get_excess_stock_list_by_id(excess_stock_list_id)
            if excess_stock_list_id
            else None
        )
        saved_mapping = None
        saved_header_row = 1
...
def process_import():
    ...
    try:
        # Get mapped indices
        sales_order_ref_index = int(
            next(key for key, value in mapping.items() if value == "sales_order_ref")
        )
        customer_code_index = int(
            next(
                key for key, value in mapping.items() if value == "customer_system_code"
            )
        )
    ...
    try:
        with db_cursor(commit=True) as cur:
            ...
            _execute_with_cursor(
                cur,
                insert_query,
                (base_part_number, raw_part_number, system_part_number),
            )
...
def start_import():
    ...
    return jsonify(
        success=True, next_step=current_step, mapping=mapping, file_id=file_id
    )


def update_import_status(
    file_id, import_type, processed, created, updated, skipped, errors, mapping=None
):
...
def import_parts():
    ...
    if not mapping or not file_id:
        return jsonify(
            success=False, message="Missing mapping or file_id", next_step="customers"
        ), 400

    file_details = get_file_by_id(file_id)
    if not file_details:
        return jsonify(
            success=False, message="File not found", next_step="customers"
        ), 404
    ...
    try:
        ...
        try:
            ...
            return jsonify(
                success=False,
                message="Required part number mappings not found",
                next_step="customers",
            ), 400
        ...
        if not system_part_number_col and not part_number_col:
            return jsonify(
                success=False,
                message="At least one part number field must be mapped",
                next_step="customers",
            ), 400
        ...
        for batch_start in range(0, len(df), batch_size):
            ...
            with db_cursor(commit=True) as cur:
                for idx in range(batch_start, batch_end):
                    ...
                    try:
                        system_part_number = (
                            str(row.iloc[int(system_part_number_col)]).strip()
                            if system_part_number_col is not None
                            else None
                        )
                        part_number = (
                            str(row.iloc[int(part_number_col)]).strip()
                            if part_number_col is not None
                            else None
                        )

                        print(
                            f"DEBUG - Raw values: system_part_number={system_part_number}, part_number={part_number}"
                        )
                        ...
                        existing_row = _execute_with_cursor(
                            cur,
                            """
                        ...
                        if existing:
                            needs_update = (
                                ...
                                or existing["system_part_number"] != system_part_number
                            )
                            if needs_update:
                                _execute_with_cursor(
                                    cur,
...
def import_customers():
    ...
    if not mapping or not file_id:
        print(
            "Missing required data - mapping:", bool(mapping), "file_id:", bool(file_id)
        )  # Debug print
        return jsonify(
            success=False,
            message=f"Missing mapping or file_id. Got mapping={mapping}, file_id={file_id}",
        ), 400
    ...
    try:
        ...
        try:
            ...
            return jsonify(
                success=False, message="Required customer mappings not found"
            ), 400

        if not system_code_col or not name_col:
            return jsonify(
                success=False, message="Both system_code and name must be mapped"
            ), 400
    ...
    try:
        ...
        with db_cursor(commit=True) as cur:
            ...
            try:
                ...
                print(
                    f"Found columns - SO Ref: {sales_order_ref_col}, Customer: {customer_system_code_col}"
                )

            except StopIteration as e:
                print("Error finding required columns:", str(e))
                return jsonify(
                    success=False, message="Required fields not found in mapping."
                ), 400
            ...
            for idx, row in df.iterrows():
                try:
                    ...
                    try:
                        ...
                        sales_order_ref = (
                            str(sales_order_ref_raw).strip()
                            if pd.notna(sales_order_ref_raw)
                            else ""
                        )
                        print(
                    ...
                    try:
                        customer_system_code_raw = row.iloc[
                            int(customer_system_code_col)
                        ]
                        customer_system_code = (
                            str(customer_system_code_raw).strip()
                            if pd.notna(customer_system_code_raw)
                            else ""
                        )
                        print(
                    ...
                    skip_reasons = []
                    if not sales_order_ref or sales_order_ref.lower() in [
                        "nan",
                        "none",
                        "",
                    ]:
                        skip_reasons.append("Missing/empty sales_order_ref")
                    if not customer_system_code or customer_system_code.lower() in [
                        "nan",
                        "none",
                        "",
                    ]:
                    ...
                    if salesperson_ref_col is not None:
                        try:
                            ...
                            if salesperson_ref:
                                salesperson_row = _execute_with_cursor(
                                    cur,
                                    """
                    ...
                    if customer_po_ref_col is not None:
                        try:
                            ...
                            customer_po_ref = (
                                str(customer_po_ref_raw).strip()
                                if pd.notna(customer_po_ref_raw)
                                else None
                            )
                            print(
                    ...
                    if date_entered_col is not None:
                        try:
                            ...
                            print(
                                f"Row {row_num}: Raw date value: '{raw_date}', type: {type(raw_date)}"
                            )
                            date_entered = validate_date(raw_date)
                            print(f"Row {row_num}: Validated date: '{date_entered}'")
                        except ValueError as e:
                            print(
                                f"Row {row_num}: Date validation error: {e}, using current date"
                            )
                            date_entered = datetime.now().strftime("%Y-%m-%d")
                        except Exception as e:
                            print(
                                f"Row {row_num}: Unexpected date error: {e}, using current date"
                            )
                            date_entered = datetime.now().strftime("%Y-%m-%d")
                    ...
                    if total_value_col is not None:
                        try:
                            ...
                            print(
                                f"Row {row_num}: Raw total value: '{raw_value}', type: {type(raw_value)}"
                            )
                            if pd.notna(raw_value):
                                ...
                                print(
                                    f"Row {row_num}: Processed total value: {total_value}"
                                )
                            else:
                    ...
                    print(
                        f"Row {row_num}: Looking up customer with system_code: '{customer_system_code}'"
                    )
                    ...
                    print(
                        f"Row {row_num}: Checking if sales order exists with ref: '{sales_order_ref}'"
                    )
                    existing_order_row = _execute_with_cursor(
                        cur,
                        """
                    ...
                    if existing_order:
                        print(
                            f"Row {row_num}: Found existing order ID {existing_order['id']}"
                        )
                        ...
                        if (
                            ...
                            and existing_order["total_value"] != total_value
                        ):
                            print(
                        ...
                        existing_salesperson_id = (
                            existing_order["salesperson_id"]
                            if existing_order["salesperson_id"] is not None
                            else 1
                        )
                        if (
                            salesperson_ref_col is not None
                            and salesperson_id != existing_salesperson_id
                        ):
                            print(
                        ...
                        if (
                            customer_po_ref
                            and existing_order["customer_po_ref"] != customer_po_ref
                        ):
                            print(
                        ...
                        if (
                            date_entered
                            and existing_order["date_entered"] != date_entered
                        ):
                            print(
                        ...
                        if needs_update:
                            ...
                            print(
                                f"Row {row_num}: Updating order with SQL: {update_sql}"
                            )
...
def import_status(file_id):
    ...
    with db_cursor() as cur:
        ...
        for result in results:
            ...
            if (
                step not in status or status[step] is None
            ):  # Only take the latest status for each step
                status[step] = {
        ...
        completed = all(
            status[step] and status[step]["completed"] for step in step_order
        )
...
def import_order_lines():
    ...
    print(
        f"Starting order lines import with mapping: {mapping}, update_existing: {update_existing}"
    )
    ...
    try:
        ...
        try:
            ...
            return jsonify(
                success=False,
                message="Required fields not found in mapping. Need sales_order_ref, system_part_number, line_number, and quantity",
            ), 400
        ...
        with db_cursor() as cur:
            ...
            all_sales_orders = {
                ...
                for row in _execute_with_cursor(
                    cur, "SELECT id, sales_order_ref FROM sales_orders"
                ).fetchall()
            }
            ...
            all_parts = {
                ...
                for row in _execute_with_cursor(
                    cur, "SELECT system_part_number, base_part_number FROM part_numbers"
                ).fetchall()
            }
            ...
            with db_cursor(commit=True) as cur:
                for idx in range(batch_start, batch_end):
                    try:
                        ...
                        # Get required fields
                        sales_order_ref = str(
                            row.iloc[int(sales_order_ref_col)]
                        ).strip()
                        system_part_number = str(
                            row.iloc[int(system_part_number_col)]
                        ).strip()
                        ...
                        try:
                            ...
                            results["errors"].append(
                                f"Row {idx + 1}: Invalid quantity format"
                            )
                            results["skipped"] += 1
                            continue
                            ...
                            try:
                                ...
                                if pd.notna(shipped_qty_value):
                                    ...
                                    if shipped_quantity > quantity:
                                        ...
                                        shipped_quantity = (
                                            quantity  # Cap at total quantity
                                        )
                            except (ValueError, TypeError) as e:
                                print(
                                    f"Error parsing shipped quantity on row {idx + 1}: {e}"
                                )

                        # Skip if any required field is missing
                        if (
                            not sales_order_ref
                            or not system_part_number
                            or not quantity
                        ):
                            results["errors"].append(
                                f"Row {idx + 1}: Missing required fields"
                            )
                            results["skipped"] += 1
                            continue
                        ...
                        if delivery_date_col is not None:
                            try:
                                ...
                                print(
                                    f"Error parsing delivery date on row {idx + 1}: {e}"
                                )
                        ...
                        if requested_date_col is not None:
                            try:
                                ...
                                print(
                                    f"Error parsing requested date on row {idx + 1}: {e}"
                                )
                        ...
                        if promise_date_col is not None:
                            try:
                                ...
                                print(
                                    f"Error parsing promise date on row {idx + 1}: {e}"
                                )
        ...
        try:
            with db_cursor(commit=True) as cur:
                ...
                orders_needing_totals = _execute_with_cursor(
                    cur,
                    """
                ...
                for order in orders_needing_totals:
                    try:
                        total_calc = _execute_with_cursor(
                            cur,
                            """
                        ...
                        calculated_total = (
                            total_calc["calculated_total"] if total_calc else 0
                        )
                        ...
                        if calculated_total > 0:
                            ...
                            print(
                                f"Order {order['sales_order_ref']}: No valid order lines with prices found"
                            )
                ...
                if calculated_count > 0:
                    print(
                        f"Automatically calculated total_value for {calculated_count} sales orders"
                    )
...
def import_progress(file_id):
    ...
    for step in steps:
        ...
        if step_status:
            ...
            is_completed = (
                step_status["completed_at"] is not None
                and step_status["status"] == "completed"
            )
            ...
            status_dict[step] = {
                ...
                "total": (step_status["processed"] or 0)
                + (step_status["skipped"] or 0),
            }
        else:
            status_dict[step] = {
                "processed": 0,
...
def import_setup():
    ...
    directory_record = db_execute(
        "SELECT directory FROM import_settings ORDER BY id DESC LIMIT 1", fetch="one"
    )
...
def save_directory():
    ...
    if not directory or not os.path.isdir(directory):
        return jsonify(
            success=False, message="Invalid directory. Please enter a valid path."
        ), 400
    if not mapping_id:
    ...
    db_execute(
        "INSERT INTO import_settings (directory, mapping_id) VALUES (?, ?)",
        (directory, mapping_id),
        commit=True,
    )

    # Start folder watcher in a background thread
    watcher_thread = threading.Thread(
        target=start_folder_watcher, args=(directory, mapping_id), daemon=True
    )
    watcher_thread.start()

    return jsonify(
        success=True, message="Directory and mapping saved. Watching for new files."
    )
...
def run_full_import(file_id, mapping_id):
    ...
    response = requests.post(
        f"{base_url}/handson/import/parts",
    ...
    response = requests.post(
        f"{base_url}/handson/import/customers",
        json={"file_id": file_id, "mapping": mapping},
    )
    response_json = response.json()
    ...
    response = requests.post(
        f"{base_url}/handson/import/sales_orders",
        json={"file_id": file_id, "mapping": mapping},
    )
    response_json = response.json()
    ...
    response = requests.post(
        f"{base_url}/handson/import/order_lines",
        json={"file_id": file_id, "mapping": mapping},
    )
    response_json = response.json()
        ...
        try:
            ...
            if df.shape[0] > 0:
                ...
                mapping_fields = [
                    ...
                    {
                        "field": "part_numbers.description",
                        "label": "Description",
                        "required": False,
                    },
                    {
                        "field": "part_numbers.manufacturer",
                        "label": "Manufacturer",
                        "required": False,
                    },
                    {
                        "field": "part_numbers.category",
                        "label": "Category",
                        "required": False,
                    },
                ]
                ...
                for idx in range(batch_start, batch_end):
                    try:
                        ...
                        if existing:
                            ...
                            if (
                                manufacturer
                                and existing["manufacturer"] != manufacturer
                            ):
        ...
        try:
            ...
            if df.shape[0] > 0:
                ...
                mapping_fields = {
                    "header": [
                        {
                            "field": "purchase_orders.po_number",
                            "label": "PO Number",
                            "required": True,
                        },
                        {
                            "field": "suppliers.fornitore",
                            "label": "Supplier Code (Fornitore)",
                            "required": True,
                        },
                        {
                            "field": "purchase_orders.date_created",
                            "label": "Date Created",
                            "required": False,
                        },
                        {
                            "field": "purchase_orders.expected_delivery",
                    ...
                    "lines": [
                        {
                            "field": "purchase_order_lines.po_number",
                            "label": "PO Number",
                            "required": True,
                        },
                        {
                            "field": "purchase_order_lines.line_number",
                            "label": "Line Number",
                            "required": True,
                        },
                        {
                        ...
                        {
                            "field": "purchase_order_lines.quantity",
                            "label": "Quantity",
                            "required": True,
                        },
                        {
                            "field": "purchase_order_lines.price",
                            "label": "Price",
                            "required": False,
                        },
                        {
                            "field": "purchase_order_lines.expected_date",
                            "label": "Expected Date",
                            "required": False,
                        },
                        {
                            "field": "purchase_order_lines.status",
                            "label": "Line Status",
                            "required": False,
                        },
                        {
                            "field": "purchase_order_lines.received_quantity",
...
def import_purchase_orders():
    ...
    try:
        ...
        try:
            row_count_df = pd.read_excel(
                _resolve_file_path(file_details["filepath"]), nrows=1
            )
            total_rows = len(
                pd.read_excel(_resolve_file_path(file_details["filepath"]), usecols=[0])
            )
        ...
        while processed_rows < total_rows or (total_rows == 0 and chunk_num == 0):
            ...
            try:
                ...
                df_chunk = pd.read_excel(
                    _resolve_file_path(file_details["filepath"]),
                    skiprows=skiprows,
                    nrows=CHUNK_SIZE,
                )
                ...
                print(
                    f"DataFrame columns in chunk {chunk_num}: {df_chunk.columns.tolist()}"
                )
                ...
                with db_cursor(commit=True) as cur:
                    ...
                    try:
                        ...
                        try:
                            ...
                            print(
                                f"Found suppliers.fornitore at column index {supplier_code_col}"
                            )
                        ...
                        return jsonify(
                            success=False,
                            message="Required fields not found in mapping.",
                        ), 400
                    ...
                    for idx, row in df_chunk.iterrows():
                        try:
                            supplier_code = str(
                                row.iloc[int(supplier_code_col)]
                            ).strip()
                            if supplier_code:
                    ...
                    if unique_supplier_codes:
                        existing_suppliers = _execute_with_cursor(
                            cur, supplier_query, unique_supplier_codes
                        ).fetchall()
                    ...
                    currencies = {}
                    currency_rows = _execute_with_cursor(
                        cur, "SELECT id, currency_code, symbol FROM currencies"
                    ).fetchall()
                    for curr in currency_rows:
                        # Use the correct column names from your table
                        currencies[curr["currency_code"].lower()] = curr[
                            "id"
                        ]  # Map by code
                    ...
                    statuses = {}
                    status_rows = _execute_with_cursor(
                        cur, "SELECT id, name FROM purchase_order_statuses"
                    ).fetchall()
                    ...
                    for idx, row in df_chunk.iterrows():
                        try:
                            ...
                            if supplier_name_col is not None:
                                try:
                                    supplier_name = (
                                        str(row.iloc[int(supplier_name_col)]).strip()
                                        or None
                                    )
                                except (ValueError, TypeError):
                                    pass
                            ...
                            if purchase_status_col is not None:
                                try:
                                    raw_status = (
                                        str(row.iloc[int(purchase_status_col)])
                                        .strip()
                                        .lower()
                                    )
                                    if raw_status:
                            ...
                            if currency_col is not None:
                                try:
                                    raw_currency = (
                                        str(row.iloc[int(currency_col)]).strip().lower()
                                    )
                                    if raw_currency:
                            ...
                            if incoterms_col is not None:
                                try:
                                    incoterms = (
                                        str(row.iloc[int(incoterms_col)]).strip()
                                        or None
                                    )
                                except (ValueError, TypeError):
                                    pass
                            ...
                            if payment_terms_col is not None:
                                try:
                                    payment_terms = (
                                        str(row.iloc[int(payment_terms_col)]).strip()
                                        or None
                                    )
                                except (ValueError, TypeError):
                                    pass
                            ...
                            if delivery_address_col is not None:
                                try:
                                    delivery_address_id = int(
                                        row.iloc[int(delivery_address_col)]
                                    )
                                except (ValueError, TypeError):
                                    pass
                            ...
                            if billing_address_col is not None:
                                try:
                                    billing_address_id = int(
                                        row.iloc[int(billing_address_col)]
                                    )
                                except (ValueError, TypeError):
                                    pass
                            ...
                            if total_value_col is not None:
                                try:
                                    ...
                                    print(
                                        f"Value conversion error on row {idx + 1}: {e}"
                                    )
                            ...
                            if not supplier_id:
                                ...
                                new_supplier = _execute_with_cursor(
                                    cur,
                                    """
                            ...
                            if existing_po:
                                ...
                                if (
                                    date_issued
                                    and existing_po["date_issued"] != date_issued
                                ):
                                ...
                                if (
                                    payment_terms
                                    and existing_po["payment_terms"] != payment_terms
                                ):
                                ...
                                if (
                                    purchase_status_id
                                    and existing_po["purchase_status_id"]
                                    != purchase_status_id
                                ):
                                ...
                                if (
                                    currency_id
                                    and existing_po["currency_id"] != currency_id
                                ):
                                ...
                                if (
                                    delivery_address_id
                                    and existing_po["delivery_address_id"]
                                    != delivery_address_id
                                ):
                                ...
                                if (
                                    billing_address_id
                                    and existing_po["billing_address_id"]
                                    != billing_address_id
                                ):
                                ...
                                if (
                                    total_value is not None
                                    and existing_po["total_value"] != total_value
                                ):
                                    update_fields.append("total_value = ?")
                                    update_values.append(total_value)
                                    needs_update = True
                            ...
                            results["skipped_details"].append(
                                {
                                    "row": idx + 1,
                                    "po_ref": purchase_order_ref
                                    if "purchase_order_ref" in locals()
                                    else "N/A",
                                    "supplier_code": supplier_code
                                    if "supplier_code" in locals()
                                    else "N/A",
                                    "reason": str(e),
                                }
                            )
        ...
        while processed_rows < total_rows or (total_rows == 0 and chunk_num == 0):
            ...
            with db_cursor(commit=True) as cur:
                ...
                if not po_ref_col:
                    return jsonify(
                        success=False,
                        message="PO Reference column not found in mapping",
                    ), 400
                ...
                if unique_po_refs:
                    ...
                    po_rows = _execute_with_cursor(
                        cur, po_query, unique_po_refs
                    ).fetchall()
                    for po in po_rows:
                ...
                if unique_part_numbers:
                    ...
                    part_rows = _execute_with_cursor(
                        cur, part_query, unique_part_numbers
                    ).fetchall()
                    for part in part_rows:
                        part_mapping[part["system_part_number"]] = part[
                            "base_part_number"
                        ]
                ...
                for idx, row in df_chunk.iterrows():
                    try:
                        ...
                        # Get required fields
                        line_number = int(
                            float(str(row.iloc[int(line_number_col)]).strip())
                        )
                        system_part_number = str(
                            row.iloc[int(system_part_number_col)]
                        ).strip()
                        ...
                        try:
                            quantity = int(
                                float(str(row.iloc[int(quantity_col)]).strip())
                            )
                            if quantity <= 0:
                                raise ValueError("Quantity must be positive")
                        except ValueError as e:
                            skip_message = (
                                f"Row {idx + 1}: Invalid quantity format: {str(e)}"
                            )
                            ...
                            results["skipped_details"].append(
                                {
                                    ...
                                    if "line_number" in locals()
                                    else "N/A",
                                    "part": system_part_number
                                    if "system_part_number" in locals()
                                    else "N/A",
                        ...
                        if status_col is not None:
                            try:
                                raw_status = (
                                    str(row.iloc[int(status_col)]).strip().lower()
                                )
                                if raw_status:
                        ...
                        if sales_order_line_col is not None:
                            try:
                                sales_order_line_id = int(
                                    row.iloc[int(sales_order_line_col)]
                                )
                            except (ValueError, TypeError):
                                pass
                        ...
                        if not po_id:
                            skip_message = (
                                f"Row {idx + 1}: Purchase order not found: {po_ref}"
                            )
                            results["errors"].append(skip_message)
                            results["skipped_details"].append(
                                {
                        ...
                        if existing_line:
                            ...
                            if (
                                promised_date
                                and existing_line["promised_date"] != promised_date
                            ):
                            ...
                            if (
                                sales_order_line_id
                                and existing_line["sales_order_line_id"]
                                != sales_order_line_id
                            ):
                            ...
                            if (
                                ...
                                and existing_line["received_quantity"]
                                != received_quantity
                            ):
...
def get_currency_id(currency_value, db=None):
    ...
    try:
        ...
        result = db_execute(
            "SELECT id FROM currencies WHERE id = ?", (currency_id,), fetch="one"
        )
        if result:
            return currency_id
    except (ValueError, TypeError):
    ...
    result = db_execute(
        "SELECT id FROM currencies WHERE UPPER(currency_code) = ?",
        (currency_value.upper(),),
        fetch="one",
    )
    if result:
    ...
    result = db_execute(
        "SELECT id FROM currencies WHERE currency_code LIKE ?",
        (f"%{currency_value}%",),
        fetch="one",
    )
    if result:
...
def get_status_id(status_value, status_type, db=None):
    ...
    try:
        ...
        # Verify the ID exists
        result = db_execute(
            f"SELECT id FROM {status_type} WHERE id = ?", (status_id,), fetch="one"
        )
        if result:
            return status_id
    except (ValueError, TypeError):
    ...
    # Try by name
    result = db_execute(
        f"SELECT id FROM {status_type} WHERE name = ?", (status_value,), fetch="one"
    )
    if result:
        return result["id"]
    ...
    result = db_execute(
        f"SELECT id FROM {status_type} WHERE code = ?", (status_value,), fetch="one"
    )
    if result:
        return result["id"]
...
def quick_import_page():
    ...
    return render_template(
        "quick_import.html", saved_mappings=[dict(m) for m in saved_mappings]
    )
...
def process_quick_import():
    ...
    try:
        ...
        with db_cursor(commit=True) as cur:
            _execute_with_cursor(
                cur, insert_query, (temp_path, os.path.basename(file_url))
            )
...
def process_parts_from_df(df, mapping, results):
    ...
    with db_cursor(commit=True) as cur:
        try:
            ...
            print(
                f"Part columns: system={system_part_number_col}, part={part_number_col}"
            )
            ...
            for system_part_number, part_info in unique_parts.items():
                ...
                try:
                    ...
                    if system_part_number in existing_by_system:
                        print(
                            f"Part {system_part_number} already exists (row {first_row}), skipping"
                        )
                        results["skipped"] += 1
                        results["processed"] += 1
                        continue
                    ...
                    if base_part_number in existing_by_base:
                        ...
                        print(
                            f"WARNING: Base part {base_part_number} exists with different system number!"
                        )
                        print(
                            f"  Existing: system={existing_info['system']}, part={existing_info['part']}"
                        )
                        print(
                            f"  New (row {first_row}): system={system_part_number}, part={part_number}"
                        )
                    ...
                    # Create new part
                    print(
                        f"Creating part: system={system_part_number}, base={base_part_number} (row {first_row})"
                    )
                    _execute_with_cursor(
                        cur,
                        """
                    ...
                    if _is_unique_violation(exc):
                        ...
                        error_msg = (
                            f"Part {system_part_number} (row {first_row}): {exc}"
                        )
                    print(error_msg)
                    results["errors"].append(error_msg)
                    results["skipped"] += 1
...
def process_customers_from_df(df, mapping, results):
    ...
    with db_cursor(commit=True) as cur:
        ...
        unique_customers = df.drop_duplicates(
            subset=[df.columns[customer_system_code_col]]
        )
        ...
        with db_cursor(commit=True) as cur:
            ...
            for order in orders_needing_totals:
                try:
                    ...
                    print(
                        f"Error calculating total for order {order['sales_order_ref']}: {e}"
                    )
...
def process_sales_orders_from_df(df, mapping, results):
    ...
    with db_cursor(commit=True) as cur:
        ...
        customer_name_col = next(
            (col for col, map_val in mapping.items() if map_val == "customers.name"),
            None,
        )
        if customer_name_col is not None:
            customer_name_col = int(customer_name_col)
        ...
        for idx, row in unique_orders.iterrows():
            try:
                ...
                if (
                    not sales_order_ref
                    or not customer_system_code
                    or sales_order_ref == "nan"
                    or customer_system_code == "nan"
                ):
                    results["skipped"] += 1
                    continue
                ...
                if not customer:
                    ...
                    print(
                        f"Creating customer: {customer_system_code} - {customer_name}"
                    )
                    _execute_with_cursor(
                        cur,
                        """
                ...
                if salesperson_ref_col is not None:
                    try:
                        ...
                        if pd.notna(salesperson_ref_raw):
                            ...
                            if salesperson_ref:
                                salesperson = _execute_with_cursor(
                                    cur,
                                    """
                                    SELECT id FROM salespeople
                                    WHERE system_ref = ?
                                ...
                                if salesperson:
                                    salesperson_id = salesperson["id"]
                                    print(
                                        f"Found salesperson ID {salesperson_id} for ref '{salesperson_ref}'"
                                    )
                                else:
                                    print(
                ...
                if customer_system_code in customer_salesperson_map:
                    ...
                    print(
                        f"Updated customer {customer_system_code} with salesperson {salesperson_id}"
                    )
                ...
                if existing:
                    ...
                    results["created"] += 1
                    print(
                        f"Created order {sales_order_ref} with salesperson_id {salesperson_id}"
                    )
...
def process_order_lines_from_df(df, mapping, results):
    ...
    with db_cursor(commit=True) as cur:
        ...
        for idx, row in df.iterrows():
            try:
                ...
                if not sales_order_ref or sales_order_ref.lower() in [
                    ...
                    results["errors"].append(
                        f"Row {idx + 1}: Missing or invalid sales order reference"
                    )
                    results["skipped"] += 1
                    continue

                if not system_part_number or system_part_number.lower() in [
                    "nan",
                    "none",
                    "",
                ]:
                    results["errors"].append(
                        f"Row {idx + 1}: Missing or invalid part number"
                    )
                    results["skipped"] += 1
                    continue
                ...
                try:
                    ...
                    results["errors"].append(
                        f"Row {idx + 1}: Invalid line number '{line_number_str}'"
                    )
                    results["skipped"] += 1
                    continue
                ...
                try:
                    ...
                    results["errors"].append(
                        f"Row {idx + 1}: Invalid quantity '{quantity_str}'"
                    )
                    results["skipped"] += 1
                    continue
                ...
                if not sales_order_id:
                    if sales_order_ref not in missing_orders:
                        ...
                        if similar:
                            print(
                                f"  Similar orders found: {[s['sales_order_ref'] for s in similar]}"
                            )
                    results["skipped"] += 1
                    continue
                ...
                if not base_part_number:
                    if system_part_number not in missing_parts:
                        print(
                            f"Row {idx + 1}: Part '{system_part_number}' not found, attempting to create..."
                        )
                        ...
                        try:
                            base_part_number = create_part_on_demand(
                                cur, system_part_number, part_number
                            )
                            all_parts[system_part_number] = (
                                base_part_number  # Update cache
                            )
                            ...
                            similar = _execute_with_cursor(
                                ...
                                """
                                           ...
                                           """,
                                (f"%{system_part_number[:5]}%",),
                            ).fetchall()
                            if similar:
                                print(
                                    f"  Similar parts found: {[s['system_part_number'] for s in similar]}"
                                )
                            results["skipped"] += 1
                            continue
        ...
        if missing_orders:
            ...
            print(
                f"Missing orders: {list(missing_orders)[:10]}{'...' if len(missing_orders) > 10 else ''}"
            )
        if missing_parts:
            print(f"\nSummary: {len(missing_parts)} unique parts could not be created")
            print(
                f"Missing parts: {list(missing_parts)[:10]}{'...' if len(missing_parts) > 10 else ''}"
            )
