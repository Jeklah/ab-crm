...
def rfqs():
    ...
    if current_user.user_type.lower() == "admin":  # Make comparison case-insensitive
        ...
        rfqs = [
            rfq
            for rfq in all_rfqs
            if str(rfq["salesperson_id"]) == str(user_salesperson_id)
        ]
    ...
    if filter_salesperson:
        rfqs = [
            rfq for rfq in rfqs if rfq.get("salesperson_id") == int(filter_salesperson)
        ]
    ...
    if base_part_number:
        rfqs = [
            ...
            for rfq in rfqs
            if any(
                base_part_number in line["base_part_number"]
                for line in get_rfq_lines(rfq["id"])
            )
        ]
    ...
    if current_user.user_type == "admin":
        ...
        customers = [
            ...
            if any(rfq["customer_id"] == c["id"] for rfq in rfqs)
        ]
    ...
    for rfq in rfqs:
        ...
        rfq["salesperson_name"] = (
            salesperson["name"] if salesperson else "No salesperson"
        )
    ...
    breadcrumbs = [("Home", "/"), ("RFQs", url_for("rfqs.rfqs"))]
...
def create_rfq():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            salesperson_id = request.form.get(
                "salesperson_id"
            )  # Get the salesperson_id from the form
            ...
            with db_cursor(commit=True) as cur:
                ...
                if customer_currency:
                    currency_id = (
                        customer_currency.get("currency_id")
                        if isinstance(customer_currency, dict)
                        else customer_currency[0]
                    )
                else:
...
def edit_rfq(rfq_id):
    ...
    for line in rfq_lines:
        price_list_info = get_price_list_price(
            line["base_part_number"], line["quantity"]
        )
        if price_list_info:
    ...
    with db_cursor() as cur:
        for line in rfq_lines:
            ...
            line["chosen_supplier_name"] = (
                ...
                else line.get("chosen_supplier_name")
            )
        ...
        for line in rfq_lines:
            ...
            previous_offers = (
                _row_value(previous_offers_row, "offer_history_count") or 0
            )
            ...
            previous_rfq_lines = (
                _row_value(previous_rfq_lines_row, "rfq_history_count") or 0
            )
            ...
            previous_sales_order_lines = (
                _row_value(previous_sales_order_lines_row, "sales_order_lines_count")
                or 0
            )
    ...
    try:
        with db_cursor(commit=True) as cur:
            ...
            if manufacturer_name:
                ...
                if mfg:
                    manufacturer_id = (
                        mfg["canonical_id"] if mfg["merged_into"] else mfg["id"]
                    )
                else:
            ...
            suggested_suppliers_str = ",".join(
                str(row["supplier_id"]) for row in suggested_suppliers
            )
...
def set_default_margin(rfq_id):
    ...
    db_execute(
        "UPDATE rfq_lines SET margin = ? WHERE rfq_id = ?",
        (default_margin, rfq_id),
        commit=True,
    )
    return redirect(url_for("rfqs.edit_rfq", rfq_id=rfq_id))
...
def set_default_supplier(rfq_id):
    ...
    db_execute(
        "UPDATE rfq_lines SET suggested_suppliers = ? WHERE rfq_id = ?",
        (default_supplier, rfq_id),
        commit=True,
    )
    return redirect(url_for("rfqs.edit_rfq", rfq_id=rfq_id))
...
def generate_requisitions(rfq_id):
    ...
    for line in rfq_lines:
        ...
        for supplier_id in suggested_suppliers:
            if supplier_id not in requisitions:
                ...
                requisitions[supplier_id] = {"reference": reference, "lines": []}
    ...
    return render_template(
        "requisitions.html", supplier_emails=supplier_emails, rfq_id=rfq_id
    )
...
def extract_data(rfq_id):
    ...
    try:
        ...
        if not request_data:
            ...
            return jsonify(
                {"success": False, "error": "'request_data' is required"}
            ), 400
        ...
        with db_cursor(commit=True) as cur:
            for part_number, quantity in extracted_lines:
                base_part_number = create_base_part_number(part_number)
                logging.debug(
                    f"Processing part: {part_number}, quantity: {quantity}, base: {base_part_number}"
                )
                ...
                logging.debug(
                    f"Inserting RFQ line: rfq_id={rfq_id}, line_number={next_line_number}, base_part_number={base_part_number}, quantity={quantity}"
                )
                _execute_with_cursor(
                    cur,
                    """
        ...
        with db_cursor(commit=True) as cur:
            ...
            for line in rfq_lines:
                ...
                try:
                    ...
                    supplier_lead_time = request.form.get(
                        f"supplier_lead_time_{line_id}", "0"
                    )
                    ...
                    base_part_number = _row_value(
                        base_part_number_row, "base_part_number"
                    )
                    ...
                    print(
                        f"Updating line {line_id}: {line_number}, {part_number}, {base_part_number}, {manufacturer_id}, {quantity}, {cost}, {supplier_lead_time}, {margin}, {price}, {lead_time}, {line_value}, {note}"
                    )
...
def get_base_cost(line_id):
    try:
        result = db_execute(
            "SELECT base_cost FROM rfq_lines WHERE id = ?", (line_id,), fetch="one"
        )
        if result:
    ...
    try:
        with db_cursor(commit=True) as cur:
            ...
            if field_name in ["margin", "quantity"]:
                ...
                if line_data:
                    base_cost = (
                        float(line_data["base_cost"]) if line_data["base_cost"] else 0
                    )
                    margin_value = (
                        float(line_data["margin"]) if line_data["margin"] else 0
                    )
                    quantity_value = (
                        float(line_data["quantity"]) if line_data["quantity"] else 0
                    )

                    if base_cost and quantity_value:
                        calculated_price = round(
                            base_cost / (1 - (margin_value / 100)), 2
                        )
                        calculated_line_value = round(
                            calculated_price * quantity_value, 2
                        )
...
def update_rfq_line_offer():
    ...
    try:
        with db_cursor(commit=True) as cur:
            ...
            exchange_rate = (
                currency_result["exchange_rate_to_eur"] if currency_result else 1
            )
            ...
            return jsonify(
                {
                    "success": True,
                    "chosen_supplier_name": updated_line["chosen_supplier_name"]
                    if updated_line
                    else None,
                    "cost_currency": updated_line["cost_currency"]
                    if updated_line
                    else None,
                    ...
                    "supplier_lead_time": updated_line["supplier_lead_time"]
                    if updated_line
                    else None,
...
def duplicate_line(line_id):
    try:
        ...
        if new_line_id:
            ...
            return jsonify(
                {"success": False, "error": "Failed to insert new line"}
            ), 500
...
def update_rfq_line_fields():
    ...
    print(
        f"line_id: {line_id}, price: {new_price}, line_value: {new_line_value}, cost: {new_cost}"
    )
    print(
        f"datecode: {new_datecode}, spq: {new_spq}, packaging: {new_packaging}, rohs: {new_rohs}, coc: {new_coc}"
    )
    ...
    try:
        ...
        with db_cursor(commit=True) as cur:
            for line in rfq_lines:
                try:
                    ...
                    if (
                        price > 0 and line.get("status_id") != 7
                    ):  # Ensure there's a price and it's not 'No Bid'
                        _execute_with_cursor(
                            cur,
                    ...
                    logging.warning(
                        f"Invalid price for line ID {line['id']}: {line['price']}"
                    )
            ...
            if all(
                status in [4, 7] for status in line_statuses
            ):  # 4 = Quoted, 7 = No Bid
                new_status = "quoted"
            else:
            ...
            _execute_with_cursor(
                cur, "UPDATE rfqs SET status = ? WHERE id = ?", (new_status, rfq_id)
            )
        ...
        with db_cursor() as cur:
            for line in rfq_lines:
                ...
                line["system_part_number"] = (
                    _row_value(system_row, "system_part_number") or ""
                )
                ...
                line["customer_part_number"] = (
                    _row_value(customer_row, "customer_part_number") or ""
                )
        ...
        return render_template(
            "quote.html", rfq=rfq, rfq_lines=quotable_lines, customer=customer
        )
    except Exception as e:
...
def quote(rfq_id):
    try:
        ...
        return render_template(
            "quote.html", rfq=rfq, rfq_lines=rfq_lines, customer=customer
        )
    except Exception as e:
            ...
            for line in rfq_lines:
                ...
                try:
                    ...
                    supplier_lead_time = safe_int(
                        request.form.get(f"supplier_lead_time_{line_id}", 0)
                    )
                    margin = float(request.form.get(f"margin_{line_id}", 0))
                    note = request.form.get(f"note_{line_id}", "")
                    chosen_supplier = request.form.get(
                        f"chosen_supplier_{line_id}", None
                    )
                    suggested_suppliers = request.form.getlist(
                        f"suggested_suppliers_{line_id}"
                    )
                    ...
                    chosen_supplier = (
                        safe_int(chosen_supplier, None) if chosen_supplier else None
                    )
                    buffer = (
                        get_supplier_buffer(chosen_supplier) if chosen_supplier else 0
                    )
                    ...
                    _execute_with_cursor(
                        ...
                        (
                            ...
                            note,
                            chosen_supplier,
                            ",".join(map(str, suggested_suppliers)),
                            line_id,
                        ),
                    )
                except Exception as e:
...
def delete_rfq(rfq_id):
    try:
        with db_cursor(commit=True) as cur:
            _execute_with_cursor(
                cur, "UPDATE rfqs SET status = ? WHERE id = ?", ("deleted", rfq_id)
            )
            _execute_with_cursor(
                cur, "UPDATE rfq_lines SET status_id = ? WHERE rfq_id = ?", (8, rfq_id)
            )
...
def bulk_update_chosen_supplier(rfq_id):
    ...
    db_execute(
        "UPDATE rfq_lines SET chosen_supplier = ? WHERE rfq_id = ?",
        (chosen_supplier, rfq_id),
        commit=True,
    )
    return redirect(url_for("rfqs.input_costs", rfq_id=rfq_id))
...
def input_costs(rfq_id):
        ...
        try:
            ...
            with db_cursor(commit=True) as cur:
                for line in rfq_lines:
                    ...
                    supplier_lead_time = safe_int(
                        request.form.get(f"supplier_lead_time_{line_id}", 0) or 0
                    )
                    ...
                    try:
                        ...
                        logging.error(
                            f"Error updating base cost for line {line_id}: {e}"
                        )
                        flash(
                            f"Error updating base cost for line {line_id}: {e}", "error"
                        )
                        continue
    ...
    try:
        ...
        for line in rfq_lines:
            ...
            line["part_number"] = (
                part_number_row["part_number"] if part_number_row else "Unknown"
            )

            if "cost_currency" not in line or line["cost_currency"] is None:
                supplier = (
                    get_supplier_by_id(line["chosen_supplier"])
        ...
        logging.debug(
            f"Final requisitioned suppliers mapping: {requisitioned_suppliers}"
        )
        ...
        for line in rfq_lines:
            ...
            logging.debug(
                f"Processing suppliers for line {line_id} with base part number {base_part_number}"
            )

            if isinstance(line.get("suggested_suppliers"), str):
                line["suggested_suppliers"] = [
                    safe_int(supplier_id)
                    for supplier_id in line["suggested_suppliers"].split(",")
                ]
            elif isinstance(line.get("suggested_suppliers"), list):
                line["suggested_suppliers"] = [
                    safe_int(supplier_id) for supplier_id in line["suggested_suppliers"]
                ]
            else:
                line["suggested_suppliers"] = []

            logging.debug(
                f"Parsed suggested suppliers for line {line_id}: {line['suggested_suppliers']}"
            )

            received_suppliers = requisitioned_suppliers.get(line_id, set())
            logging.debug(
                f"Received suppliers for line {line_id}: {received_suppliers}"
            )

            line["received_requisitions"] = [
                supplier
                for supplier in suppliers
                if supplier["id"] in received_suppliers
            ]

            line["not_received_requisitions"] = [
                supplier
                for supplier in suppliers
                if supplier["id"] not in received_suppliers
            ]
            logging.debug(
                f"Not received suppliers for line {line_id}: {[supplier['id'] for supplier in line['not_received_requisitions']]}"
            )

            sorted_suppliers = line["received_requisitions"] + [
                supplier
                for supplier in line["not_received_requisitions"]
                if supplier["id"] in line["suggested_suppliers"]
            ]
            sorted_suppliers += [
                supplier
                for supplier in line["not_received_requisitions"]
                if supplier["id"] not in line["suggested_suppliers"]
            ]
        ...
        for line in rfq_lines[:5]:
            logging.debug(
                f"Line ID: {line['id']}, Base Part Number: {line['base_part_number']}, "
        ...
        if first_line:
            logging.debug(
                f"Line ID: {first_line['id']}, Base Part Number: {first_line['base_part_number']}"
            )
            logging.debug(
...
def decode_content(part):
    ...
    )  # Print the first 100 bytes of the raw content for debugging
    ...
    if charset is not None:
        try:
            ...
            print(
                "Decoded content with detected charset:", decoded_content[:100]
    ...
    for encoding in ["utf-8", "iso-8859-1", "windows-1252", "ascii"]:
        try:
            ...
            print(
                f"Decoded content with {encoding}:", decoded_content[:100]
            )  # Print the first 100 chars
            return decoded_content
        except UnicodeDecodeError as e:
    ...
    print(
        "Decoded content with utf-8 replace:", decoded_content[:100]
    )  # Print the first 100 chars
    return decoded_content
...
def clean_html(html_content):
    ...
    return bleach.clean(
        cleaned_html, tags=allowed_tags, attributes=allowed_attributes, strip=True
    )
...
def add_part_number():
    ...
    part = (
        session.query(PartNumber).filter_by(base_part_number=base_part_number).first()
    )
    if not part:
        new_part = PartNumber(
            base_part_number=base_part_number,
...
def update_rfq_status_based_on_lines(rfq_id):
    lines = (
        db_execute(
            "SELECT status_id FROM rfq_lines WHERE rfq_id = ?", (rfq_id,), fetch="all"
        )
        or []
    )
    if not lines:
...
def update_rfq_status(rfq_id, status_text):
    db_execute(
        "UPDATE rfqs SET status = ? WHERE id = ?", (status_text, rfq_id), commit=True
    )
...
def mark_as_no_bid():
    ...
    db_execute(
        "UPDATE rfq_lines SET status_id = 7 WHERE id = ?", (line_id,), commit=True
    )
...
def edit_rfq_line(rfq_id, line_id):
    ...
    return render_template(
        "edit_rfq_line.html", rfq=rfq, line=line, manufacturers=manufacturers
    )
...
def add_requisition_to_top_level(
    top_level_requisition_id, requisition_id, max_retries=5, delay=0.1
):
    insert_query = """
    ...
    for attempt in range(max_retries):
        try:
            with db_cursor(commit=True) as cur:
                _execute_with_cursor(
                    cur, insert_query, (top_level_requisition_id, requisition_id)
                )
            return
    ...
    try:
        with db_cursor(commit=True) as cur:
            for line in line_data:
                ...
                logging.debug(
                    f"Processing Line ID: {line_id}, RFQ ID: {rfq_id}, Line Data: {line_details}"
                )
                ...
                insert_row = _execute_with_cursor(
                    ...
                    (
                        ...
                        supplier_id,
                        datetime.now().strftime("%Y-%m-%d"),
                        line_details["base_part_number"],
                        line_details["quantity"],
                        line_id,
                    ),
                    fetch="one" if _using_postgres() else None,
                )
                ...
                if top_level_requisition_id:
                    ...
                    logging.debug(
                        f"Inserted requisition reference for top-level requisition ID: "
...
def get_requisition_details():
    ...
    try:
        ...
        quantities = [
            str(row["quantity"]) if row["quantity"] is not None else "0" for row in rows
        ]
...
def generate_requisitions_ajax(rfq_id):
    ...
    for attempt in range(max_retries):
        try:
            top_level_reference = (
                f"TLR-{rfq_id}-{datetime.now().strftime('%Y%m%d%H%M%S')}"
            )
            created_at = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            returning_clause = " RETURNING id" if _using_postgres() else ""
            insert_top_query = f"""
            ...
            with db_cursor(commit=True) as cur:
                ...
                for line in rfq_lines:
                    ...
                    for supplier_id in suggested_suppliers:
                        ...
                        _execute_with_cursor(
                            cur, update_rfq_line_status_query, (2, line["id"])
                        )

                        supplier = next(
                            (s for s in suppliers if s["id"] == supplier_id), None
                        )
                        if supplier:
                            supplier_entry = supplier_emails.setdefault(
                                supplier["name"],
                                {
                                    "contact_name": supplier["contact_name"],
                                    "contact_email": supplier["contact_email"],
                    ...
                    remaining_suppliers = [
                        s
                        for s in suggested_suppliers
                        if int(s) not in processed_suppliers
                    ]
                    new_suggested_suppliers = (
                        ",".join(remaining_suppliers) if remaining_suppliers else None
                    )
    ...
    return jsonify(
        {"success": False, "error": "Database locked after maximum retries"}
    ), 500
...
def get_rfq_line_currency(line_id):
    ...
    try:
        ...
        rfq_result = db_execute(
            "SELECT currency FROM rfqs WHERE id = ?", (rfq_id,), fetch="one"
        )
        ...
        logging.debug(
            f"Currency data: cost_currency={cost_currency}, exchange_rate={exchange_rate_to_eur}, "
...
def update_rfq_currency(rfq_id):
    ...
    try:
        db_execute(
            "UPDATE rfqs SET currency = ? WHERE id = ?",
            (currency_id, rfq_id),
            commit=True,
        )
        return jsonify({"success": True})
...
def get_currency_exchange_rate_route(currency_id):
    ...
    try:
        ...
        return jsonify(
            {"success": True, "exchange_rate_to_eur": result["exchange_rate_to_eur"]}
        )
    except Exception as e:
...
def update_base_cost():
    ...
    if (
        not line_id
        or cost is None
        or not isinstance(cost_currency, int)
    ...
    try:
        ...
        db_execute(
            ...
            WHERE id = ?
        """,
            (base_cost, cost, cost_currency, line_id),
            commit=True,
        )

        return jsonify(
            success=True, message="Base cost updated successfully", base_cost=base_cost
        )
    except Exception as e:
...
def update_rfq_line_with_price_list():
    ...
    current_line = db_execute(
        "SELECT * FROM rfq_lines WHERE id = ?", (line_id,), fetch="one"
    )
...
def get_history(line_id):
    try:
        base_part_number_row = db_execute(
    ...
    try:
        ...
        if margin is not None:
            ...
            if (1 - (update_data["margin"] / 100)) == 0:
                return jsonify(
                    {"success": False, "error": "Margin too high, division by zero."}
                ), 400
        ...
        update_rfq_line_db(
            line_id=line_id,
            update_data={
                "chosen_supplier": update_data.get(
                    "chosen_supplier", rfq_line.get("chosen_supplier")
                ),
                "price": update_data.get("price", rfq_line.get("price")),
                "supplier_lead_time": update_data.get(
                    "lead_time", rfq_line.get("supplier_lead_time", 0)
                ),
...
def upload_email_body(rfq_id):
    ...
    try:
        db_execute(
            "UPDATE rfqs SET email = ? WHERE id = ?", (email_body, rfq_id), commit=True
        )
        logging.info(f"Email content updated for RFQ {rfq_id}")
        return jsonify(
            {
                "status": "success",
                "message": "Email content uploaded and processed successfully",
            }
        ), 200
    except Exception as e:
...
def delete_rfq_line(line_id):
    try:
        with db_cursor(commit=True) as cur:
            _execute_with_cursor(
                cur, "UPDATE rfq_lines SET status_id = ? WHERE id = ?", (8, line_id)
            )
...
def rfq_excel_view(rfq_id):
    ...
    return render_template(
        "rfq_excel_view.html", rfq=rfq, rfq_lines_json=rfq_lines_json
    )
    ...
    if request.method == "POST":
        ...
        with db_cursor(commit=True) as cur:
            for entry in part_numbers_data:
                ...
                results.append(
                    f"Added/Updated part number {part_number} for RFQ {rfq_id}"
                )
...
def view_and_edit_rfq(rfq_id):
    if request.method == "POST":
        ...
        insert_manufacturer_query = (
            f"INSERT INTO manufacturers (name) VALUES (?){returning_clause}"
        )
        insert_line_query = """
        ...
        try:
            with db_cursor(commit=True) as cur:
                rfq = _execute_with_cursor(
                    cur, "SELECT * FROM rfqs WHERE id = ?", (rfq_id,), fetch="one"
                )
                if not rfq:
                    return jsonify({"success": False, "error": "RFQ not found"}), 404

                _execute_with_cursor(
                    cur, "DELETE FROM rfq_lines WHERE rfq_id = ?", (rfq_id,)
                )
                ...
                for entry in part_numbers_data:
                    ...
                    if manufacturer_name:
                        manufacturer = _execute_with_cursor(
                            cur, manufacturer_query, (manufacturer_name,), fetch="one"
                        )

                        if manufacturer:
                            manufacturer_id = (
                                manufacturer["canonical_id"] or manufacturer["id"]
                            )
                        else:
    ...
    """
    rfq_lines = (
        db_execute(rfq_lines_query, (rfq["customer_id"], rfq_id), fetch="all") or []
    )
    ...
    return render_template("view_and_edit_rfq.html", rfq=rfq, rfq_lines=existing_lines)
...
def get_rfq_lines_quick_view(rfq_id):
    ...
    for row in rows:
        result_lines.append({"part_number": row[1], "quantity": row[2]})
...
def update_line_part_number():
    ...
    try:
        ...
        db_execute(
            ...
            WHERE id = ?
        """,
            (base_part_number, line_id),
            commit=True,
        )

        return jsonify({"success": True, "display_part_number": display_part_number})
    except Exception as e:
        return jsonify({"success": False, "error": str(e)})
...
def create_rfq_from_bom():
    ...
    customer_currency = db_execute(
        "SELECT currency_id FROM customers WHERE id = ?", (customer_id,), fetch="one"
    )
    ...
    for component in data["components"]:
        if not component.get("base_part_number"):
            return jsonify(
                {"error": "Each component must have a base_part_number"}
            ), 400
        try:
            component["quantity"] = (
                int(component["quantity"]) if component.get("quantity") else 1
            )
        except ValueError:
            return jsonify(
                {
                    "error": f"Invalid quantity for part {component.get('base_part_number')}"
                }
            ), 400
...
def create_rfq_from_project():
    ...
    try:
        ...
        currency_id = (
            ...
            else (customer[0] if customer else 1)
        )
    ...
    try:
        ...
        for line in rfq_lines:
            ...
            if "cost_currency" not in line or line["cost_currency"] is None:
                ...
                line["cost_currency"] = (
                    supplier["currency"] if supplier else 3
                )  # Default to EUR
        ...
        suppliers_json = json.dumps(
            [convert_to_dict(supplier) for supplier in suppliers]
        )
        statuses_json = json.dumps([convert_to_dict(status) for status in statuses])
        currencies_json = json.dumps(
            [convert_to_dict(currency) for currency in currencies]
        )

        return render_template(
            "pricing_playground.html",
            rfq_id=rfq_id,
...
def save_pricing():
    ...
    try:
        ...
        logging.info(
            f"Successfully updated pricing for {updated_count} lines of RFQ ID: {rfq_id}"
        )
...
def chase_rfq():
    ...
    try:
        ...
        return jsonify(
            {"success": True, "message": "RFQ marked as chased", "update": new_update}
        )
...
def rfq_playground(rfq_id):
    ...
    for line in rfq_lines:
        ...
        line["offer_history"] = (
            ...
            LIMIT 5
        """,
                (line["base_part_number"], line.get("offer_id")),
                fetch="all",
            )
            or []
        )
