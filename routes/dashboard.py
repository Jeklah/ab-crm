...
from models import dict_from_row, get_db
...
def view_dashboard():
    ...
    international_sales_pct = (
        (international_sales / total_sales * 100) if total_sales > 0 else 0
    )

    # Get saved queries and panels as before
    saved_queries = conn.execute("SELECT id, query_name FROM saved_queries").fetchall()
    panels = conn.execute(
        "SELECT * FROM dashboard_panels ORDER BY panel_order"
    ).fetchall()

    new_customer_count = (
        new_customers_data["new_customer_count"] if new_customers_data else 0
    )
    new_customer_sales = (
        new_customers_data["new_customer_sales"] if new_customers_data else 0
    )
...
def create_panel():
    ...
    try:
        ...
        return jsonify({"success": True, "panel_id": cursor.lastrowid})
