...
def insert_purchase_order(supplier_id):
    ...
    row = db_execute(
        ...
        (
            ...
            if supplier and "payment_terms" in supplier
            else None,
            1,
...
def insert_purchase_order_line(
    purchase_order_id,
    line_number,
    base_part_number,
    quantity,
...
def update_sales_order(sales_order_id, customer_id, customer_po_ref, sales_status_id):
    db_execute(
        ...
        WHERE id = ?
    """,
        (customer_id, customer_po_ref, sales_status_id, sales_order_id),
        commit=True,
    )
...
def get_purchase_order_id_from_line(line_id):
    row = db_execute(
        "SELECT purchase_order_id FROM purchase_order_lines WHERE id = ?",
        (line_id,),
        fetch="one",
    )
...
def update_sales_order_line_ship_date(so_line_id, new_ship_date):
    db_execute(
        ...
        WHERE id = ?
    """,
        (new_ship_date.strftime("%Y-%m-%d"), so_line_id),
        commit=True,
    )
...
def generate_sales_order_acknowledgment(sales_order):
    ...
    response.headers["Content-Disposition"] = (
        f"inline; filename=sales_order_{sales_order['id']}_acknowledgment.pdf"
    )
    ...
    pdf_canvas.drawString(
        100, 800, f"Sales Order Acknowledgment - #{sales_order['id']}"
    )
...
def generate_sales_order_acknowledgment_file(sales_order):
    acknowledgment_row = db_execute(
        "SELECT COUNT(*) as count FROM acknowledgments WHERE sales_order_id = ?",
        (sales_order["id"],),
        fetch="one",
    )
    ...
    file_path = (
        f"./static/pdfs/sales_order_{sales_order['id']}_acknowledgment_v{version}.pdf"
    )
...
def get_sales_order_lines_with_rfq_options(sales_order_id):
    ...
    for row in rows:
        line = dict(row)
        line["rfq_options"] = get_rfq_lines_for_part_and_customer(
            line["base_part_number"], line["customer_id"]
        )
        result.append(line)
    return result
...
def insert_purchase_order_line_from_suggestion(purchase_order_id, sales_order_line_id):
    ...
    if not rfq_line_data:
        raise ValueError(
            f"No RFQ line data found for sales order line ID: {sales_order_line_id}"
        )
    ...
    if rfq_line_data["price"] is None:
        ...
        if not price_result or price_result["price"] is None:
            raise ValueError(
                f"No price found for sales order line ID: {sales_order_line_id}"
            )
    ...
    """
    next_line_number_row = db_execute(
        line_number_query, (purchase_order_id,), fetch="one"
    )
    next_line_number = (
        next_line_number_row["next_line_number"] if next_line_number_row else 1
    )
...
def get_addresses_by_customer(customer_id):
    return query_all(
        "SELECT * FROM customer_addresses WHERE customer_id = ?", (customer_id,)
    )
...
def update_project(
    project_id, customer_id, salesperson_id, name, description, status_id
):
    db_execute(
        """
        UPDATE projects
...
def get_project_stages(project_id):
    ...
    for stage in stages:
        ...
        # Also ensure empty strings are treated as None
            ...
            stage.get("description") is not None and stage["description"].strip() == ""
        ):
            stage["description"] = None
...
def delete_project_stage(stage_id):
    db_execute(
        "DELETE FROM project_stages WHERE id = ? OR parent_stage_id = ?",
        (stage_id, stage_id),
        commit=True,
    )
...
def get_project_stage_by_id(stage_id):
    row = db_execute(
        "SELECT * FROM project_stages WHERE id = ?", (stage_id,), fetch="one"
    )
...
def has_substages(stage_id):
    row = db_execute(
        "SELECT COUNT(*) as count FROM project_stages WHERE parent_stage_id = ?",
        (stage_id,),
        fetch="one",
    )
...
def get_customers_with_tags_single_query(
    salesperson_id,
    search_term=None,
    status_filter=None,
    sort_column="name",
...
def insert_customer_industry(customer_id, industry_id):
    db_execute(
        "INSERT INTO customer_industries (customer_id, industry_id) VALUES (?, ?)",
        (customer_id, industry_id),
        commit=True,
    )
...
def insert_customer_tags(customer_id, tags):
    with db_cursor(commit=True) as cur:
        for tag in tags:
            ...
            tag_row = cur.execute(
                "SELECT id FROM industry_tags WHERE tag = ?", (tag,)
            ).fetchone()
            if not tag_row:
                cur.execute(
                    "INSERT INTO industry_tags (tag) VALUES (?) RETURNING id", (tag,)
                )
                inserted = cur.fetchone()
                tag_id = (
                    inserted["id"]
                    if inserted and isinstance(inserted, dict) and "id" in inserted
                    else (list(inserted.values())[0] if inserted else None)
                )
            else:
...
def delete_customer_tags(customer_id):
    db_execute(
        "DELETE FROM customer_industry_tags WHERE customer_id = ?",
        (customer_id,),
        commit=True,
    )
...
def update_customer_industry(customer_id, industry_id):
    with db_cursor(commit=True) as cur:
        cur.execute(
            "DELETE FROM customer_industries WHERE customer_id = ?", (customer_id,)
        )
        cur.execute(
            "INSERT INTO customer_industries (customer_id, industry_id) VALUES (?, ?)",
            (customer_id, industry_id),
...
def delete_customer_industries(customer_id):
    db_execute(
        "DELETE FROM customer_industries WHERE customer_id = ?",
        (customer_id,),
        commit=True,
    )


def insert_rfq_from_macro(
    customer_id, contact_id, customer_ref, currency, status, email_content=None
):
    try:
        row = db_execute(
...
def get_part_number_by_id(base_part_number):
    part_number = query_one(
        "SELECT * FROM part_numbers WHERE base_part_number = ?", (base_part_number,)
    )
...
def save_email_file_and_create_entries(file, rfq_id=None):
    ...
    with db_cursor(commit=True) as cur:
        ...
        if email_content and rfq_id:
            cur.execute(
                "UPDATE rfqs SET email = ? WHERE id = ?", (email_content, rfq_id)
            )
...
def get_latest_activity(customer_id):
    ...
    latest_values = [
        _first_value_from_row(row)
        for row in (rfq_row, sales_order_row, update_row)
        if row
    ]
    ...
    if latest_activity_date:
        logging.debug(
            "Customer %s Final Latest Activity: %s", customer_id, latest_activity_date
        )
...
def get_breadcrumbs(stage):
    ...
    while current_stage:
        ...
        )  # Assuming 'parent_stage' is a reference to the parent stage
    return breadcrumbs
...
def get_stage(stage_id):
    try:
        ...
        updates = (
            db_execute(
                "SELECT * FROM stage_updates WHERE stage_id = ?",
                (stage_id,),
                fetch="all",
            )
            or []
...
def get_stage_by_id(stage_id):
    ...
    return {
        ...
        "parent_stage": {"id": row["parent_stage_id"], "name": row["parent_stage_name"]}
        if row.get("parent_stage_id")
        else None,
    }
...
def process_email_and_attachments(record_id, table_name, file):
    try:
        ...
        with db_cursor(commit=True) as cur:
            ...
            cur.execute(
                f"INSERT INTO {table_name}_files ({table_name}_id, file_id) VALUES (?, ?)",
                (record_id, file_id),
            )

            cur.execute(
                f"UPDATE {table_name} SET email = ? WHERE id = ?",
                (email_content, record_id),
            )
            ...
            for attachment in msg.attachments:
                ...
                attachment_path = os.path.join(
                    current_app.config["UPLOAD_FOLDER"], attachment_filename
                )
                ...
                attachment_file_id = (
                    attachment_row["id"]
                    if isinstance(attachment_row, dict) and "id" in attachment_row
                    else (attachment_row[0] if attachment_row else None)
                )
                ...
                cur.execute(
                    f"INSERT INTO {table_name}_files ({table_name}_id, file_id) VALUES (?, ?)",
                    (record_id, attachment_file_id),
                )
                attachments.append(
                    {"filename": attachment_filename, "filepath": attachment_path}
                )
...
def get_excess_list_by_id1(list_id):
    row = db_execute(
        "SELECT * FROM excess_stock_lists WHERE id = ?", (list_id,), fetch="one"
    )
...
def get_excess_stock_list_by_id(excess_list_id):
    ...
    return {
        ...
        "mapping_header_row": row.get("mapping_header_row")
        if isinstance(row, dict)
        else row["mapping_header_row"],
    }
...
def insert_excess_stock_line(
    excess_stock_list_id,
    base_part_number,
    quantity,
    date_code=None,
    manufacturer=None,
    unit_price=None,
    unit_price_currency_id=None,
    part_number=None,
):
    db_execute(
        """
...
def get_excess_stock_list_id_by_file(file_id):
    try:
        ...
        logging.error(
            "Error fetching excess_stock_list_id for file_id %s: %s", file_id, e
        )
        return None
...
def get_excess_list_line_by_id(line_id):
    row = db_execute(
        "SELECT * FROM excess_list_lines WHERE id = ?", (line_id,), fetch="one"
    )
    return dict(row) if row else None
...
def match_rfq_lines(excess_stock_list_id):
    ...
    for line in excess_lines:
        ...
        if rfq_lines:
            matches.append({"excess_line": line, "rfq_lines": rfq_lines})
...
def match_sales_order_lines(excess_stock_list_id):
    ...
    for line in excess_lines:
        ...
        if sales_order_lines:
            matches.append(
                {"excess_line": line, "sales_order_lines": sales_order_lines}
            )
...
def get_supplier_by_email(email):
    row = db_execute(
        "SELECT * FROM suppliers WHERE contact_email = ?", (email,), fetch="one"
    )
    return dict(row) if row else None
...
def get_customer_statuses():
    rows = (
        db_execute("SELECT id, status FROM customer_status ORDER BY id", fetch="all")
        or []
    )
    return [dict(row) for row in rows]
...
# Load country name mapping from the data/ subdirectory.
# The file was moved from the project root to data/ during the cleanup (Jan 2026).
# We use Path(__file__) to make the path work regardless of the current working
# directory, which can vary between development and production launches.
import pathlib as _pathlib

_data_dir = _pathlib.Path(__file__).resolve().parent.parent / "data"
_mapping_path = _data_dir / "country_name_mapping.json"
# Fall back to the old root-level location so existing deployments that have not
# yet moved the file continue to work without an immediate file-system change.
if not _mapping_path.exists():
    _mapping_path = (
        _pathlib.Path(__file__).resolve().parent.parent / "country_name_mapping.json"
    )
        ...
        countries.append(
            {
                "code": country_code,
                "name": country_name,
                "display_name": f"{country_name} ({country_code})"
...
def get_countries_by_continent() -> Dict[str, List[str]]:
    ...
    continent_mapping = {
        ...
        "North America": ["CA", "US", "MX", "GL", "BM", "PM"],
        "South America": [
            "AR",
            "BO",
            "BR",
...
def get_continents() -> List[str]:
    ...
    """
    return ["Europe", "North America", "South America", "Asia", "Africa", "Oceania"]
